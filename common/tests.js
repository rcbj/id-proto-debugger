...
// in the test container), so a bare
// require("selenium-webdriver")/require("jsonwebtoken") here would fail to
// resolve. Instead the caller — which already has these in scope — injects the
// Selenium primitives (By, until, Select), the bunyan logger, the configured
// waitTime, and the jwt/assert helpers. Usage:
//
...
module.exports = ({ By, until, Select, waitTime, log, jwt, assert }) => {
  ...
  // something the caller can do anything about. These two live here rather than
  // in one test because two tests have now been bitten by the same window:
  // tests/oidc_userinfo.js immediately after "Return to debugger", and
  // tests/oauth2_token_revocation.js on its SECOND revocation, where the
  // re-render triggered by the first revocation's saveOperationToHistory()
  // landed between findElement() and the click.
  //
  ...
  // Read a field's value, tolerating a re-render. The window is easy to miss on
  // a READ: findElements() and getAttribute() are two round trips to the
  // browser with a re-render able to land in between. Returns null when the
  // field genuinely is not there, which callers already handle.
    ...
    for (;;) {
      try {
        ...
        if (Date.now() >= deadline ||
            !/stale element/i.test(e.message)) throw e;
        await pause(100);
      }
    }
  }

  // Click something, tolerating a re-render. The element is located afresh
  // inside the loop rather than passed in, because a WebElement handle is
      ...
      try {
        ...
        if (found.length) {
          await driver.executeScript(
              "arguments[0].scrollIntoView({block:'center'});", found[0]);
          await found[0].click();
        ...
        // Stale, detached, or momentarily not interactable: the page is
        // mid-render.
        last = e;
      }
      await pause(150);
  ...
  async function populateMetadata(driver, discovery_endpoint) {
    ...
    const btn_oidc_discovery_endpoint =
        By.className("btn_oidc_discovery_endpoint");
    const btn_oidc_populate_meta_data =
        By.className("btn_oidc_populate_meta_data");
    ...
    await driver.wait(until.elementIsVisible(driver.findElement(
                      oidc_discovery_endpoint)), waitTime);

    // Enter discovery endpoint
    await driver.findElement(oidc_discovery_endpoint).clear();
    await driver.findElement(oidc_discovery_endpoint)
    ...
    // made this flaky. Retrieve fetches the discovery document asynchronously
    // and the Populate button is rendered only when it arrives, so waiting for
    // that button does gate on the fetch. But the click itself can still land
    // without effect — the button is re-rendered as the table is built, so a
    // click can reach the copy that is being replaced — and the handler leaves
    // every field untouched when it runs against an empty document (jQuery's
    // .val(undefined) is a no-op, so the seeded placeholders simply remain).
    // One click and a bare wait therefore fails with "still reads
    // https://localhost/..." roughly once per cold run, which is what happened
    // to the OAuth2 Client Credentials job on 2026-08-05.
    //
    ...
    while (Date.now() < deadline && !filled) {
      ...
      if (buttons.length) {
        try {
          ...
          // Replaced between find and click. The next pass picks up the new
          // one.
          log.debug("populateMetadata(): the Populate button went stale; " +
                    "retrying. " + e.message);
        }
      }
      try {
        await driver.wait(async function () {
          ...
          // Filled, and no longer the seeded placeholder. Checked this way
          // rather than against an expected URL so the one condition covers
          // Keycloak's realms, the mock STS and a deployed OP alike.
        ...
        log.debug("populateMetadata(): endpoints not populated yet after " +
                  clicks + " click(s).");
      }
    }
    if (!filled) {
      throw new Error("Metadata retrieval did not populate the " +
                      "endpoints after " + clicks +
                      " Populate click(s): #authorization_endpoint " +
                          "still reads \"" + seen +
                      "\". The discovery document is " + discovery_endpoint +
                          ".");
    }
    log.info("Leaving populateMetadata(). authorization_endpoint=" + seen);
    log.debug("Leaving populateMetadata().");
  }
  ...
  //                          { access_token, refresh_token } (asserting a
  //                          refresh token was issued); otherwise return
  //                          access_token.
  async function getAccessTokenAuthCode(driver, client_id, client_secret, scope,
                                        pkce_enabled, {
    baseUrl,
    grantType = "OIDC Authorization Code Flow(code)",
    returnRefreshToken = false,
  } = {}) {
    log.debug("Entering getAccessTokenAuthCode().");
    ...
    await new Select(await driver.findElement(authorization_grant_type))
                     .selectByVisibleText(grantType);
    await driver.wait(until.elementLocated(usePKCE_yes), waitTime);
    await driver.wait(until.elementIsVisible(driver.findElement(usePKCE_yes)),
                      waitTime);
    await driver.wait(until.elementLocated(usePKCE_no), waitTime);
    await driver.wait(until.elementIsVisible(driver.findElement(usePKCE_no)),
                      waitTime);
    ...
    try {
      ...
      await driver.wait(until.elementIsVisible(driver.findElement(
                        keycloak_username)), waitTime);
    } catch (error) {
      log.error("Unable to log into keycloak.");
      const authz_error_report =
          await driver.findElement(By.id("authz-error-report"));
      const authz_error_report_paragraphs =
          await authz_error_report.findElements(By.css("p"));
      throw new Error(await authz_error_report_paragraphs[
                      authz_error_report_paragraphs.length - 1].getText());
    }
    ...
    await driver.findElement(token_redirect_uri).sendKeys(baseUrl +
                             "/callback");
    await driver.findElement(token_btn).click();

    // Wait for whichever appears first: the access token field or the error
    // field.
  ...
  async function getAccessTokenClientCredentials(driver, client_id,
    ...
    try {
      ...
      log.debug("Leaving getAccessTokenClientCredentials().");
      return await driver.findElement(visibleAccessTokenElement)
                                      .getAttribute("value");
    } catch (e) {
  ...
  async function getAccessTokenImplicit(driver, client_id, scope, { baseUrl } =
                                        {}) {
    log.debug("Entering getAccessTokenImplicit().");
    ...
    const keycloak_password = By.id("password");
    const keycloak_kc_login = By.id("kc-login");
    // The Authorization Endpoint Results pane's own field. It is
    // authz_access_token and not the token endpoint pane's token_access_token
    // because both panes can be on the page at once, and an implicit flow's
    // token is only ever in this one.
    ...
    await new Select(await driver.findElement(authorization_grant_type))
                     .selectByVisibleText('OAuth2 Implicit Grant');

    // Expand the authorization section and wait for the client_id field to
    // render
    await driver.wait(until.elementLocated(authz_expand_button), waitTime);
    await driver.wait(until.elementIsVisible(driver.findElement(
                      authz_expand_button)), waitTime);
    ...
    // Wait for the Keycloak login form, reporting any authorization error if it
    // never appears
    try {
      await driver.wait(until.elementLocated(keycloak_username), waitTime);
      await driver.wait(until.elementIsVisible(driver.findElement(
    ...
    log.info("Begin returning token.");
    log.debug("Leaving getAccessTokenImplicit().");
    return await driver.findElement(visibleAccessTokenElement)
                                    .getAttribute("value");
  }
  ...
  async function getAccessTokenPassword(driver, client_id, client_secret, scope,
    ...
    await driver.executeScript("arguments[0].scrollIntoView({ behavior: " +
                               "'smooth', block: 'center' });",
                               await driver.findElement(noCheckOIDCArtifacts));
    ...
    await driver.executeScript("arguments[0].scrollIntoView({ behavior: " +
                               "'smooth', block: 'center' });",
                               await driver.findElement(usePKCENo));
    ...
    if (configBtnVal === "Hide") {
      await driver.executeScript("arguments[0].scrollIntoView({ behavior: " +
                                 "'smooth', block: 'center' });", configBtnEl);
      await configBtnEl.click();
    }

    // Select POST auth style
    const token_postAuthStyleCheckToken =
        By.id("token_postAuthStyleCheckToken");
    await driver.wait(until.elementLocated(token_postAuthStyleCheckToken),
                      waitTime);
    await driver.executeScript("arguments[0].scrollIntoView({ behavior: " +
        "'smooth', block: 'center' });",
        await driver.findElement(token_postAuthStyleCheckToken));
    ...
    await driver.wait(until.elementLocated(token_initiateFromBackEnd),
                      waitTime);
    await driver.executeScript("arguments[0].scrollIntoView({ behavior: " +
        "'smooth', block: 'center' });",
        await driver.findElement(token_initiateFromBackEnd));
    ...
    await driver.executeScript("arguments[0].scrollIntoView({ behavior: " +
                               "'smooth', block: 'center' });",
                               await driver.findElement(token_btn));
    await driver.findElement(token_btn).click();
    ...
    log.info("Begin returning token.");
    log.debug("Leaving getAccessTokenPassword().");
    return await driver.findElement(visibleAccessTokenElement)
                                    .getAttribute("value");
  }

  // Decode the access token JWT and assert its claims. The base assertions
  // (decodable, client id claim, scope) apply everywhere; the rest are opt-in
  // so each grant type asserts only what it expects.
  //
  // options:
  ...
  //   verifyIdentityClaims — assert given_name/family_name/email (default
  //   true).
  ...
  async function verifyAccessToken(access_token, client_id, scope, {
    ...
    async function compareScopes(scope1, scope2) {
      log.debug("Entering compareScopes().");
      scope1 = scope1.split(" ");
      scope2 = scope2.split(" ");

      log.debug("Leaving compareScopes().");
    ...
    assert.notStrictEqual(decoded_access_token, null,
                          "Cannot decode access token. Request result: " +
                          (response_text ?
                          response_text[1] : "no response text"));
    assert.strictEqual(decoded_access_token.payload[clientIdClaim], client_id,
                       "Access token " + clientIdClaim +
                       " does not match client ID.");
    assert.strictEqual(await compareScopes(decoded_access_token.payload.scope,
                       scope), true,
                       "Access token scope does not match scope.");
    if (user !== undefined) {
      assert.strictEqual(decoded_access_token.payload.sub, user,
          "Access token SUB does not match user ID: access_token.payload.sub=" +
          decoded_access_token.payload.sub + " , user=" + user);
    }
    if (audience !== undefined) {
      assert.strictEqual(decoded_access_token.payload.aud, audience,
                         "Access token aud does not match " + audience);
    }
    if (issuer !== undefined) {
      assert.strictEqual(decoded_access_token.payload.iss, issuer,
                         "Access token iss does not match " + issuer);
    }
    if (verifyIdentityClaims) {
      assert.strictEqual(decoded_access_token.payload.given_name, client_id,
                         "Access token given_name does not match.");
      assert.strictEqual(decoded_access_token.payload.family_name, client_id,
                         "Access token family_name does not match.");
      assert.strictEqual(decoded_access_token.payload.email,
                         `${client_id}@iyasec.io`,
                         "Access token email does not match.");
    }
    if (verifyTyp) {
      assert.strictEqual(decoded_access_token.payload.typ, "Bearer",
                         "Access Token typ does not match.");
    }
    log.debug("Leaving verifyAccessToken().");
  }
