...
| **the debugger EMBEDDED in the mock STS** — `embedded/build.sh` and `embedded/Dockerfile`, which build the tree that service serves and forks; `DEPLOYMENT=embedded` in `client/build.js`; the two `env/embedded.js` files; the api's unix-socket listener in `common/tls_listener.js`; and `allowedAddressRanges`, the SSRF guard's allow-list mode. **The mock STS is the consumer and the file is the contract** — a renamed variable or path there breaks the other repository | `embedded/CLAUDE.md` |
...
- **`/client/src/pqc_x509.js`** — the **PKIX binding of the post-quantum algorithms**, and it is a third module rather than more rows in `pqc.js` because the same algorithm is a different set of BYTES in the two worlds: a JOSE ML-DSA key is an AKP JWK whose `pub` is base64url, and an X.509 one is a `SubjectPublicKeyInfo` whose BIT STRING is the same octets with no wrapping at all. It carries the OIDs and encodings of **RFC 9881** (ML-DSA), **RFC 9909** (SLH-DSA) and **RFC 9935** (ML-KEM, which signs nothing and is therefore a SUBJECT key only), and **Composite ML-DSA** from draft-ietf-lamps-pq-composite-sigs-19 — sixteen of its eighteen algorithms, the two brainpool ones being absent because the curves are in no library here and inventing one is not on. Two things surprise everybody once and both are the specifications: the private key is an ASN.1 **CHOICE** whose three arms (`seed`, `expandedKey`, `both`) are all legal, the RFC recommends the first and OpenSSL 3.5 writes the third — so this writes `seed` and reads all three; and the composite's ECDSA half keeps its `0x04` point prefix and uses a DER signature here while JOSE does neither, which is three one-line differences that each produce a signature nothing else verifies. **No DOM**, so `tests/pki_pqc_x509.js` drives every path in node and hands the result to **OpenSSL 3.5 through node's own crypto** rather than to the `openssl` BINARY, whose version nothing here pins — an Ubuntu 22.04 host has 3.0, which has no post-quantum algorithms at all and refuses an ML-DSA certificate with a message about OpenSSL rather than about the certificate. See `docs/pki.md`.
- **`/sts/`** — A mock Security Token Service used by the test suite (OAuth2 AS, OIDC OP, WS-Trust, **WS-Federation IdP**, OID4VCI issuer, OID4VP verifier, DID publisher, and — on two HTTPS listeners of its own — a **TLS / mutual-TLS endpoint** whose whole content is what the server saw of the connection, which is what the PKI page presents a client certificate to). **Its code is no longer in this repository** — it is the [`rcbj/iya-sts`](https://github.com/rcbj/iya-sts) submodule, so `git submodule update --init --recursive sts` is required once per checkout — **`--recursive`, because the mock STS has a submodule of its own** (`node-ldapjs`, which its package.json takes as `"ldapjs": "file:node-ldapjs"`), and an uninitialised submodule is an empty DIRECTORY, so plain `--init` builds an image whose container dies at startup with `Cannot find module 'ldapjs'`, a message naming a package rather than a submodule. An edit under `sts/` is an edit to somebody else's checkout. Since 2026-08-19 its **`/admin` console has a management API beside it at `/admin-api`** — the same pages and the same forms over JSON, with a generated OpenAPI 3.1 document at `/admin-api/openapi.json` and an explorer that calls it at `/admin-api/docs` — which is how a test pins what that service issues without driving a form. **It was unprotected like everything else there until the 2026-09-09 bump and is not any more**: every call into it presents an OAuth 2.0 access token audienced to that API, carrying `admin:read` to read and `admin:write` to write, minted by the seeded `sts-management-api` client — whose secret this repository's launchers pin per run (`ADMIN_API_CLIENT_SECRET`, generated in `common/common.sh`) because it is otherwise readable only THROUGH the API it unlocks. `tests/tools/admin-api-token.js` mints one and `tests/tools/attach-admin-token.js` puts it on the fifteen jobs that drive that API; `adminApi.authRequired=false` restores the open API exactly. The mock's own `admin_api.js` asserts the rule it is written under: a control added to the console gets an operation in the same commit. **That test, and the three beside it that drove the `/admin` console, left this repository's suite on 2026-08-28** — they assert things about the mock rather than about this debugger, so they are the submodule's now. See `docs/mock-sts.md`.
...
**The mock STS is the exception, and on the containerized test stack it is
not root any more either.** Its image is the `sts/` submodule's, not this
repository's, and it binds 88 (TCP and UDP), 389 and 636 — all privileged — so
on `docker-compose.yml`, `local-tests.yml` and `keycloak-tests.yml` it still
runs as the image's root. `docker-compose-run-tests.yml` runs it as **uid
10001** with `sysctls: net.ipv4.ip_unprivileged_port_start: 0`, which is per
network namespace and so lowers the bar inside that container alone. The
reason is the SPIRE Server API's Unix socket, which the tests container dials
and which reads as a protocol failure when it goes wrong: since iya-sts
27b81c5 (2026-09-12) the mock chmods that socket **0600** once bound, because
it is the trusted `local` entity, and `connect(2)` needs WRITE — so a root
mock refused the tests container with an EACCES that `spiffe_protocol.js`
reports inside a gRPC error. Until the 2026-09-15 bump the fix was `umask
0000` through an `entrypoint:` wrapper; a chmod ignores the umask, so the
same uid on both sides is the arrangement now, and the socket's volume is a
tmpfs owned by 10001 (Docker makes a plain named volume root:root, where a
uid-10001 mock cannot bind).
