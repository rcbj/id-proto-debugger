...
**A third, `allowedAddressRanges`, inverts the policy and is set by exactly one configuration** — `env/embedded.js`, when the mock STS embeds this debugger and passes `DEBUGGER_ALLOWED_ADDRESS_RANGES` (see `embedded/CLAUDE.md`). A non-empty list REPLACES the block-list (whose first entry, loopback, is usually what an allow-list names), is on whatever `blockPrivateNetworkCalls` says, and refuses everything outside it through the same `enabled` / `blockedRangeFor()` pair the four raw-socket relays already ask, so none of them needed an edit. **An allow-list with no usable entry fails CLOSED**, the opposite of the block-list's empty case, because a typo must not open the relay. Absent or `[]`, nothing above changes. `tests/api_ssrf_guard.js`'s allow-list section holds all of it.
