...
**Branded Google Chrome will not side-load an unpacked extension.** It refuses the flags and says so only on stderr (`--disable-extensions-except is not allowed in Google Chrome, ignoring`), after which the extension is simply absent and every assertion times out naming nothing. Chrome for Testing (what the tests image pins) and Chromium allow it. Independently, **chromedriver passes `--disable-extensions` among its default switches**, which cancels `--load-extension` on its own — `excludeSwitches("disable-extensions")` is required too. `run-report.js` probes EVERY browser it knows how to find and, when none of them can side-load, **fetches one**: Selenium Manager — the binary already inside `selenium-webdriver` — downloads a Chrome for Testing and its matching chromedriver into `~/.cache/selenium`, and the job is pinned to both (`CHROME_BIN` and `CHROMEDRIVER_BIN`, since a browser and a driver from different releases fail on a version mismatch). Until 2026-08-18 it answered with the first browser it found and skipped, which is a permanent skip on every host whose `google-chrome` is the branded build — i.e. on `./local-run-tests.sh`, the launcher most people run. It skips only when there is no capable browser and no network to fetch one.
