...
| | |
|---|---|
| `tests/api_ldap.js` | the protocol, node only: the ten operations, the three outcomes, the scopes compared against each other, membership from both ends, the negatives, the api's own refusals, the auto-created user, a populated directory at 20 users / 10 groups, and the one fixture it leaves behind |
