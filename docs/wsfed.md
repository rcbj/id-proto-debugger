...
Since 2026-08 the **mock STS answers this profile too** (`sts/ws-federation/wsfed.js`, and `docs/mock-sts.md`), and **every combination below runs against both** — 42 jobs where there were 21. That is not redundancy; the two IdPs are complementary, and the reason is what each of them *cannot* tell you:
...
The differences between them are four, all marked `IDP:` in the test, and none is a difference in the protocol: the submit button's id on the sign-in screen (`#kc-login` vs `#wsfed-login` — the fields are `#username`/`#password` on both), where the passive endpoint sits relative to the metadata document, which token types an inline `wreq` may ask for, and **how sign-out returns the browser** (see below). The first of those four is now a difference of ONE, and the test says so by trying both ids: the mock stopped drawing a screen of its own on 2026-08-26 and hands a `wsignin1.0` with no session to `authn.js` like the other three browser SSO profiles, so its button is `#kc-login` too and `#wsfed-login` is the side-car's alone. What that bought is three features that live in the shared funnel and were inert for this profile — federation, `fedAuthnMechanism`, and the WebAuthn step in either role; see `tests/CLAUDE.md` on the federation grid. Anything else that diverges is a finding rather than a case to special-case, which is the point of running both.
