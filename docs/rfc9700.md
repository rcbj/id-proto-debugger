...
[rcbj/iya-sts issue #2](https://github.com/rcbj/iya-sts/issues/2).
...
**iya-sts closed that in 2026-08-25** by marking `oauth2.rfc9700`
...
iya-sts's own `config.js` says so on the `global.https` row.
...
submodule — a PATH test, which silently took its else branch when iya-sts
...
trusting the PEM the endpoint hands back, done one step earlier. iya-sts says
so at `/tls`.
...
**What it waits on.** The `sts/` submodule must point at an iya-sts commit
