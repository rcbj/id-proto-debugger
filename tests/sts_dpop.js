...
function claimsOf(token) {
  ...
  return jsonFromB64u(String(token).split(".")[1]);
}
...
function athOf(token) {
  ...
  return crypto.createHash("sha256").update(String(token),
...
function makeProof(key, opts) {
  log.debug("Entering makeProof().");
  var over = opts.over || {};
  var header = Object.assign({ typ: "dpop+jwt", alg: key.alg,
      jwk: key.publicJwk },
                             over.header || {});
  ...
  var signingInput = b64u(JSON.stringify(header)) + "." +
...
async function post(url, opts) {
  ...
  try {
    ...
    // Not JSON. That is itself information for a caller checking an error
    // shape, so it is reported rather than thrown.
    parsed = null;
  }
...
async function bearerToken() {
  log.debug("Entering bearerToken().");
  var r = await post(TOKEN_ENDPOINT, {
    form: { grant_type: "client_credentials", client_id: CLIENT_ID,
           scope: "openid" }
  });
  assert.strictEqual(r.status, 200, "could not get a Bearer token: " +
                     r.text.slice(0, 200));
  log.debug("Leaving bearerToken().");
  return r.body;
}

async function boundToken(key, extra) {
  log.debug("Entering boundToken().");
  var proof = makeProof(key, { htm: "POST", htu: TOKEN_ENDPOINT,
      nonce: (extra || {}).nonce });
  var r = await post(TOKEN_ENDPOINT, {
    form: Object.assign({ grant_type: "client_credentials",
                        client_id: CLIENT_ID, scope: "openid" },
                        (extra || {}).form || {}),
    headers: { DPoP: proof }
  });
  log.debug("Leaving boundToken().");
  return r;
}
...
async function serverAdvertisesDpop() {
  ...
  log.info("=== The authorization server advertises DPoP (RFC 9449 " +
           "section 5.1) ===");
  ...
  assert.ok(Array.isArray(algs) && algs.length,
    "dpop_signing_alg_values_supported must be advertised: it is the ONLY " +
        "signal a wallet has " +
    "that DPoP is on offer, so a server that supports it silently will never " +
        "be asked.");
  assert.ok(algs.indexOf("ES256") >= 0, "ES256 must be among them; got " +
            algs.join(", "));
  assert.strictEqual(algs.indexOf("none"), -1,
                     "none must never be advertised.");
  assert.strictEqual(algs.indexOf("HS256"), -1,
    "no MAC may be advertised — the server would hold the same secret and " +
        "could mint proofs.");
...
async function bearerStillWorks() {
  ...
  assert.strictEqual(token.token_type, "Bearer",
    "a Token Request with no DPoP header must yield a Bearer token — adding " +
        "DPoP must not " +
...
async function tokenIsBoundToTheProofKey() {
  ...
  log.info("=== The Token Request binds the token (RFC 9449 sections " +
           "5, 6.1) ===");
  var key = newKey("ec");
  var r = await boundToken(key);
  assert.strictEqual(r.status, 200, "a valid DPoP proof was refused: " +
                     r.text.slice(0, 300));
  assert.strictEqual(r.body.token_type, "DPoP",
    "token_type must be DPoP, not Bearer: it is how the wallet learns it " +
        "must send a proof on " +
    "every later call. Got " + r.body.token_type);
  var claims = claimsOf(r.body.access_token);
  assert.ok(claims.cnf && claims.cnf.jkt,
    "the access token must carry cnf.jkt — inside the signed token, so a " +
        "resource server can " +
    "check the binding without asking anybody.");
  assert.strictEqual(claims.cnf.jkt, jkt(key),
    "cnf.jkt must be the RFC 7638 thumbprint of the key that signed the " +
        "proof, computed here " +
    "by an independent implementation. Got " + claims.cnf.jkt + ", expected " +
        jkt(key));
  log.info("[token] OK — token_type DPoP and cnf.jkt is this client's own " +
           "thumbprint.");

  // An RSA proof must work too, or the advertised algorithm list is a lie.
  var rsa = newKey("rsa");
  var r2 = await boundToken(rsa);
  assert.strictEqual(r2.status, 200, "an RS256 DPoP proof was refused: " +
                     r2.text.slice(0, 200));
  assert.strictEqual(claimsOf(r2.body.access_token).cnf.jkt, jkt(rsa),
    "an RSA key's thumbprint uses e/kty/n — a different member list from an " +
        "EC key's, and a " +
...
async function tokenEndpointRefusesBadProofs() {
  ...
  var cases = [
    ["check 4: typ", { header: { typ: "JWT" } },
     "some other JWT the client signed with the same key would be accepted " +
         "as a proof — and " +
     "this very workflow signs one, the OID4VCI openid4vci-proof+jwt"],
    ["check 5: alg none", { header: { alg: "none" } },
     "an unsigned proof proves nothing"],
    ...
    ["check 7: private key in the header",
     { header: { jwk: Object.assign({ d: "AAAA" },
       key.publicJwk) } },
           "RFC 9449 forbids private material in the jwk header"],
    ["check 3: no jti", { remove: ["jti"] },
     "replay detection has nothing to work with"],
    ["check 3: no htm", { remove: ["htm"] },
     "the proof is not tied to a method"],
    ["check 3: no htu", { remove: ["htu"] },
     "the proof is not tied to an endpoint"],
    ...
    ["check 9: the wrong htu", { payload: { htu: stsBase +
     "/oid4vci/credential" } },
     "THE credential endpoint's proof must not be usable at the " +
         "token endpoint"],
    ["check 11: a stale iat", { payload: { iat: Math.floor(Date.now() / 1000) -
     4000 } },
     "an old proof stays usable forever"],
    ["check 11: an iat in the future",
     { payload: { iat: Math.floor(Date.now() / 1000) + 4000 } },
     "a proof can be minted for later use"],
    ["check 2: not a JWT", { notAJwt: true },
     "a malformed proof must be refused, not crash"]
  ];

  for (const [label, over, why] of cases) {
    var proof = over.notAJwt ? "this-is-not-a-jwt"
                             : makeProof(key, { htm: "POST",
                                         htu: TOKEN_ENDPOINT, over: over });
    var r = await post(TOKEN_ENDPOINT, {
      form: { grant_type: "client_credentials", client_id: CLIENT_ID,
             scope: "openid" },
      headers: { DPoP: proof }
    ...
    assert.strictEqual(r.status, 400,
      label + ": this proof must be REFUSED (400), got " + r.status +
          ". Otherwise " + why + ".");
  ...
  log.info("[token] OK — " + cases.length +
           " bad proofs each refused with invalid_dpop_proof.");
  ...
  assert.strictEqual(doubled.status, 400,
    "check 1: two DPoP header fields must be refused; accepting either would " +
        "let an attacker " +
  ...
  assert.strictEqual(stillFine.status, 200,
    "after all those refusals a VALID proof must still be accepted — " +
...
async function proofsAreSingleUse() {
  ...
  var key = newKey("ec");
  var proof = makeProof(key, { htm: "POST", htu: TOKEN_ENDPOINT });
  var form = { grant_type: "client_credentials", client_id: CLIENT_ID,
      scope: "openid" };
  var first = await post(TOKEN_ENDPOINT, { form: form,
      headers: { DPoP: proof } });
  assert.strictEqual(first.status, 200, "the first use of a proof must work.");
  var second = await post(TOKEN_ENDPOINT, { form: form,
      headers: { DPoP: proof } });
  assert.strictEqual(second.status, 400,
    "the SAME proof must not work twice — without jti replay detection a " +
        "captured proof stays " +
    "usable for its whole iat window, which is the difference between a " +
        "proof of possession " +
    "and a password.");
  assert.ok(/already been used|jti/i.test(JSON.stringify(second.body)),
    "and the refusal should say it was a replay. Got: " + second.text.slice(0,
        200));
...
async function boundTokenIsUsableAtTheCredentialEndpoint(bound) {
  ...
  log.info("=== The Credential Endpoint accepts the binding (RFC 9449 " +
           "section 7) ===");
  var key = bound.key;
  ...
  assert.notStrictEqual(ok.status, 401,
    "a correct DPoP presentation was rejected as unauthorized: " +
        ok.text.slice(0, 300));
  ...
  assert.strictEqual(asBearer.status, 401,
    "a DPoP-BOUND token presented as a Bearer token must be refused. The " +
        "bytes are the same, " +
    "so a server that accepts either has thrown the binding away entirely " +
        "and DPoP has bought " +
    "it nothing.");
  assert.ok(/DPoP/.test(asBearer.authenticate),
    "and the challenge must name DPoP so the client knows what to do. Got: " +
        asBearer.authenticate);
  log.info("[resource] OK — the bound token is refused when presented " +
           "as Bearer.");
  ...
  assert.strictEqual(athFail.status, 401,
    "check 12: a proof whose ath hashes a DIFFERENT token must be refused. " +
        "Without this a " +
    "proof captured alongside one token can be presented with a stolen one, " +
        "which is exactly " +
  ...
  assert.strictEqual(noAthResult.status, 401,
    "check 12: ath is REQUIRED when the proof accompanies an access token, " +
        "so a proof without " +
    "one must be refused rather than treated as an older client.");
  log.info("[resource] OK — a proof with no ath at all is refused.");

  // cnf.jkt: the client's own key must not be believed over the token's
  // binding.
  ...
  assert.strictEqual(wrongKey.status, 401,
    "check 12: the proof key must match the token's cnf.jkt. This is the " +
        "assertion that says " +
    "the token is bound to SOMETHING — without it a thief signs with their " +
        "own key and is " +
    "believed, and every other check still passes.");
  log.info("[resource] OK — a proof from the wrong key is refused (cnf.jkt " +
           "enforced).");
  ...
  assert.strictEqual(htuFail.status, 401,
    "check 9: a proof made for the token endpoint must not be accepted at " +
        "the credential " +
    "endpoint.");
  ...
  assert.notStrictEqual(bearerOk.status, 401,
    "an UNBOUND token must still be accepted as Bearer — the endpoint has " +
        "not simply started " +
    "refusing everything. Got " + bearerOk.status + ": " +
        bearerOk.text.slice(0, 200));
...
async function notificationEndpointIsProtectedToo() {
  ...
  assert.strictEqual(asBearer.status, 401,
    "the notification endpoint must refuse a bound token presented as " +
  ...
  log.info("[resource] OK — the notification endpoint enforces the same rule " +
           "(401 as Bearer, " +
...
async function refreshTokenCarriesTheBinding() {
  ...
  assert.ok(refreshClaims.cnf && refreshClaims.cnf.jkt === jkt(key),
    "a refresh token issued alongside a bound access token must itself be " +
        "bound: a wallet is a " +
    "public client, so an unbound refresh token would be a bearer credential " +
        "that mints bound " +
  ...
  var wrongKeyProof = makeProof(newKey("ec"), { htm: "POST",
      htu: TOKEN_ENDPOINT });
  ...
  assert.strictEqual(withWrongKey.status, 400,
    "nor by a different key — otherwise a stolen refresh token is laundered " +
        "into one bound to " +
  ...
  assert.strictEqual(refreshed.status, 200,
                     "the right key must be able to refresh: " +
  ...
  log.info("[refresh] OK — redeemable only by its own key, and the binding " +
           "survives.");
...
async function nonceHandshakeWorks() {
  ...
  try {
    ...
    // or a conforming client has no way forward.
    ...
    assert.strictEqual(asked.body.error, "use_dpop_nonce",
      "the error must be use_dpop_nonce — a plain invalid_dpop_proof leaves " +
          "a correct client " +
    ...
    assert.strictEqual(retried.status, 200,
      "the retry carrying the server's nonce must succeed: " +
          retried.text.slice(0, 200));
    log.info("[nonce] OK — the retry is accepted.");

    // An invented nonce must not do.
    var invented = await boundToken(newKey("ec"),
        { nonce: "a-nonce-nobody-issued" });
    assert.strictEqual(invented.status, 400,
      "a nonce the server never issued must be refused, or the claim " +
          "checks nothing.");
    ...
    assert.strictEqual(nonceResponse.status, 200,
                       "the OID4VCI nonce endpoint did not answer.");
    assert.ok(nonceResponse.body.c_nonce, "it must still return a c_nonce.");
    assert.ok(nonceResponse.dpopNonce,
      "and with nonce mode on it should also carry DPoP-Nonce — this is the " +
          "one thing OID4VCI " +
      "says about DPoP by name, and the only place in the flow where the 401 " +
          "handshake can be " +
    ...
    assert.strictEqual(bound.status, 200,
                       "could not get a bound token for the resource test.");
    var noNonceProof = makeProof(key, {
      htm: "POST", htu: CREDENTIAL_ENDPOINT,
          accessToken: bound.body.access_token
    });
    var rs = await post(CREDENTIAL_ENDPOINT, {
      json: { credential_configuration_id: "IdentityCredential" },
      headers: { Authorization: "DPoP " + bound.body.access_token,
                DPoP: noNonceProof }
    });
    assert.strictEqual(rs.status, 401,
      "a resource server requiring a nonce answers 401, not 400. Got " +
          rs.status);
    assert.ok(/use_dpop_nonce/.test(rs.authenticate),
      "and says use_dpop_nonce in WWW-Authenticate rather than in a JSON " +
          "body. Got: " +
      rs.authenticate);
    ...
    log.info("[nonce] OK — the resource server's 401/WWW-Authenticate shape " +
             "works too.");
  } finally {
...
async function authorizationCodeCanBeBoundToTheKey() {
  ...
  assert.strictEqual(noProof.status, 400,
    "a code bound with dpop_jkt must not be redeemable without a DPoP proof. " +
        "This is the window " +
    "PKCE does not close: a thief with the code AND the code_verifier still " +
        "cannot sign for the " +
  ...
  assert.strictEqual(wrong.status, 400,
    "a code bound to one key must not be redeemable by another. Got " +
        wrong.status);
  ...
  assert.strictEqual(plain.status, 200,
    "an authorization request WITHOUT dpop_jkt must still redeem with no " +
        "proof — otherwise the " +
  ...
  log.info("[dpop_jkt] OK — the nominated key redeems it and the token " +
           "is bound.");
...
async function test() {
  ...
  var reachable = await fetch(stsBase +
...
program
  .name("sts_dpop")
  .description("Verify the STS mock's DPoP support (RFC 9449) at the token " +
