...
// an independently written OpenID4VP 1.0 verifier with DCQL — instead of the
// mock Verifier the STS service hosts.
//
...
//   2. walt.id's management API creates a verification session with a DCQL
//      query, exactly as its portal would;
...
//
// POSITIVE: the flow above. NEGATIVE: the same flow with a claim the verifier
// asked for withheld — walt.id must not report success, and its own DCQL/policy
// machinery is what refuses it.
//
// Needs the waltid-verifier container (WALTID_VERIFIER_URL), the waltid-issuer
// container (WALTID_ISSUER_URL) and Keycloak — walt.id's issuer authenticates
// the End-User there. Skipped when WALTID_VERIFIER_URL is unset, like the
// issuer's interoperability job.
...
// walt.id's two services, each behind its own CORS proxy. These are the
// addresses the BROWSER uses, which is also what walt.id publishes in
// everything it builds.
var verifierBase = (process.env.WALTID_VERIFIER_URL ||
    "http://localhost:7003").replace(/\/+$/, "");
var issuerBase = (process.env.WALTID_ISSUER_URL ||
    "http://localhost:7005").replace(/\/+$/, "");
// walt.id's Credential Issuer Identifier has a path; its metadata therefore
// lives at the RFC 8414 path-inserted location, which is what our wallet has to
// derive.
var issuerId = issuerBase + "/openid4vci";
var metadataUrl = issuerBase +
...
var CONFIGURATION_ID = process.env.WALTID_CONFIGURATION_ID ||
    "identity_credential";
...
// What walt.id will be asked to verify. Two claims, both selectively
...
const { text, value, waitForStatus, waitForValue, waitForFilled } =
       require("./wait_for");
...
async function issueFromWaltid(driver) {
  ...
  // which this function used to do — makes the whole configuration step
  // vacuous: the retrieval failed, the endpoint fields stayed empty, and the
  // first thing that actually noticed was a 30-second timeout waiting for a
  // Keycloak login screen that nothing had navigated to.
  ...
  assert.strictEqual(await value(driver, "vci_credential_issuer"), issuerId,
    "the metadata should name walt.id's Credential Issuer Identifier (with " +
        "its path).");
  ...
  // identifier HAS a path, so the metadata is at the RFC 8414 path-INSERTED
  // URL. The page derives that itself; do not overwrite it here. Appending
  // instead (…/openid4vci/.well-known/oauth-authorization-server) is a 404,
  // which is the very bug the issuance suite exists to keep fixed.
  var asUrl = await value(driver, "oidc_discovery_endpoint");
  assert.strictEqual(asUrl, issuerBase +
                     "/.well-known/oauth-authorization-server/openid4vci",
    "the wallet should fall back to the credential issuer at the " +
        "path-inserted well-known URL. Got: " +
    asUrl);
  ...
  // is why the old version of this function timed out at the login screen
  // rather than reporting what was actually wrong.
  assert.strictEqual(await value(driver, "authorization_endpoint"), issuerId +
                     "/authorize",
    "the authorization endpoint should have come from walt.id's RFC 8414 " +
        "document.");
  ...
  await driver.wait(until.elementLocated(By.id("username")), fetchWait,
    "walt.id should have sent the browser to Keycloak to authenticate the " +
        "End-User.");
  ...
  assert.strictEqual(header.alg, "ES256",
    "walt.id signs SD-JWT VCs with ES256 (our mock uses RS256). Got: " +
        header.alg);
  assert.ok(payload.cnf && payload.cnf.jwk,
    "and it should be bound to the holder key our wallet generated.");
  var disclosures =
      credential.split("~").slice(1).filter(Boolean).map(function (d) {
    return JSON.parse(b64uDecode(d).toString("utf8"))[1];
  });
  REQUESTED.forEach(function (name) {
    assert.ok(disclosures.indexOf(name) !== -1,
      "the credential must carry " + name +
          " as a Disclosure for this test to mean anything. Got: " +
      disclosures.join(", "));
  });
  log.info("[phase1] OK — walt.id issued a " + payload.vct + " (alg " +
           header.alg + ", iss " +
           String(payload.iss).slice(0, 24) + "…) with Disclosures: " +
                  disclosures.join(", "));
...
// "bootstrap" one (client_id + request_uri) and the full one (by value) — which
// is what lets this suite exercise our step 1 both ways.
// ---------------------------------------------------------------------------
// The `meta` member is REQUIRED in a DCQL credential query (OID4VP section
// 6.1), and for SD-JWT VC it is the vct the verifier will accept. It is read
// off the credential walt.id just issued rather than guessed: walt.id derives
// the vct from its own baseUrl, so hardcoding it here would be a second place
// to get wrong.
function dcqlQuery(vct, claims) {
  log.debug("Entering dcqlQuery().");
  log.debug("Leaving dcqlQuery().");
  return {
    credentials: [{
      id: DCQL_ID,
      format: "dc+sd-jwt",
      meta: { vct_values: [vct] },
      claims: (claims ||
               REQUESTED).map(function (name) { return { path: [name] }; })
    }]
  };
}
...
async function createVerificationSession(opts) {
  ...
  assert.ok((opts || {}).vct,
    "a verification session needs the vct to ask for; pass the one " +
        "walt.id issued.");
  var setup = {
  ...
  assert.ok(created.body.fullAuthorizationRequestUrl ||
            created.body.bootstrapAuthorizationRequestUrl,
    "and an Authorization Request URL for the wallet. Got: " +
        created.raw.slice(0, 300));
  log.debug("Leaving createVerificationSession(). sessionId=" +
            created.body.sessionId);
  return created.body;
}

function sessionInfo(sessionId) {
  log.debug("Entering sessionInfo().");
  log.debug("Leaving sessionInfo().");
  return httpJson(verifierBase + "/verification-session/" +
                  encodeURIComponent(sessionId) + "/info");
}
...
function requestQuery(session, byReference) {
  log.debug("Entering requestQuery().");
  var url = byReference
  ...
  var q = s.indexOf("?");
  assert.ok(q > 0,
      "walt.id's Authorization Request URL should carry a query string. Got: " +
      s);
  log.debug("Leaving requestQuery().");
  return s.slice(q + 1);
}
...
async function presentToWaltid(driver, session, opts) {
  ...
  assert.ok(/Request read/.test(requestStatus),
    "our wallet should be able to answer walt.id's request. Got: " +
        requestStatus +
    " (client_id " + request.clientId + ", response_mode " +
  ...
  assert.ok(request.nonce && request.nonce !== "—",
            "with a nonce to bind the presentation to.");
  ...
  assert.ok(((dcql.credentials[0].meta || {}).vct_values || []).length,
    "and the vct it will accept, which our step 1 checks against the " +
        "credential in hand.");
  ...
  log.info("[phase3] walt.id asks (" + request.source + "): client_id " +
           request.clientId +
           ", claims " + askedFor.join(", ") + "; signature: " +
               request.signature);
  ...
  var selected = await driver.executeScript(
    ...
    "  .filter(function (tr) { return " +
        "tr.querySelectorAll('td')[0].querySelector('input').checked; })" +
  ...
  assert.deepStrictEqual(selected.slice().sort(), askedFor.slice().sort(),
    "the wallet should default to exactly the claims walt.id asked for. Got: " +
        selected.join(", "));
  if (withhold) {
    await driver.executeScript(
      ...
      "  if (td[1].textContent.trim() === arguments[0]) { " +
          "td[0].querySelector('input').click(); return; }" +
      "}", withhold);
  ...
  assert.strictEqual(kbPayload.nonce, request.nonce,
                     "and carry the nonce from its request.");
  ...
  assert.strictEqual(kbPayload.sd_hash,
    ...
    "sd_hash must be the hash of the issuer-signed JWT and the presented " +
        "Disclosures, each followed " +
    "by a tilde.");
  log.info("[phase3] built an SD-JWT+KB with " + (parts.length - 2) +
           " Disclosure(s) for walt.id.");
  ...
  var sent = await driver.executeScript(
    "return { status: " +
        "document.getElementById('vp_sent_status').textContent.trim()," +
    "         recheck: " +
        "document.getElementById('vp_recheck_status').textContent.trim()," +
    "         presentation: " +
        "document.getElementById('vp_sent_presentation').value };");
  assert.ok(/all pass/.test(sent.recheck),
    "the wallet's own checks on what it sent should pass. Got: " +
        sent.recheck);
  log.info("[phase3] " + sent.status.replace(/\s+/g, " ").slice(0, 140));
  log.debug("Leaving presentToWaltid().");
  return { presentation: sent.presentation, request: request,
          askedFor: askedFor };
}
...
async function waltidVerdict(sessionId, label) {
  ...
  // walt.id validates asynchronously: the session moves through its states
  // after the POST is answered, so the record is polled rather than read once.
  while (Date.now() < deadline) {
    info = await sessionInfo(sessionId);
    assert.ok(info.ok, "walt.id should report its session. Got HTTP " +
              info.status);
    var status = String(info.body.status || "");
    if (/SUCCESS|FAIL|ERROR|COMPLETE|REJECT/i.test(status)) break;
    if (info.body.presented_credentials || info.body.policy_results ||
        info.body.failure) break;
    await new Promise(function (r) { setTimeout(r, 1000); });
  }
  log.info("[waltid] " + label + " — session " + sessionId + " status=" +
           JSON.stringify(info.body.status) +
           (info.body.failure ? ", failure=" +
            JSON.stringify(info.body.failure).slice(0, 200) : ""));
...
async function positiveFlow(driver, held, byReference) {
  ...
  var presented = await presentToWaltid(driver, session,
  ...
  assert.ok(!verdict.failure,
    "and should record no failure. Got: " + JSON.stringify(verdict.failure ||
        null).slice(0, 300));

  // What walt.id ended up with. Its record carries the raw presentation and the
  // credentials it parsed out of it; the claims are asserted against the ones
  // it asked for, which is the whole point of selective disclosure.
  var raw = JSON.stringify(verdict.presented_credentials ||
      verdict.presented_raw_data ||
                           verdict.presented_presentations || {});
  REQUESTED.forEach(function (name) {
    assert.ok(raw.indexOf(name) !== -1,
      "walt.id should have received " + name + ". Its record: " + raw.slice(0,
          400));
  });
  ["email", "phone_number"].forEach(function (name) {
    assert.ok(raw.indexOf(name) === -1,
      "and must NOT have received " + name +
          " — it was never disclosed. Its record: " + raw.slice(0, 400));
  });
  // Every policy walt.id ran on our presentation, and how it went.
  var policies = JSON.stringify(verdict.policy_results || {});
  assert.ok(policies.indexOf("false") === -1 ||
            policies.indexOf("success") !== -1,
    "walt.id's policy results should not report a failure: " + policies.slice(0,
        400));
  log.info("[positive] OK — walt.id accepted a presentation of a credential " +
           "it issued, with " +
...
//
// The presentation is otherwise perfect: the issuer's signature, the digests
// and the Key Binding JWT are all valid, and our wallet's own checks pass. What
// is wrong is that it does not answer the question.
//
...
// asked for is simply absent from what it recorded, and its status is
// SUCCESSFUL anyway.
//
...
async function negativeWithheldClaim(driver, held) {
  ...
  var verdict = await waltidVerdict(session.sessionId, "negative (withheld " +
      REQUESTED[0] + ")");

  // Whatever it decided, the withheld claim must not have reached it: that is
  // the guarantee selective disclosure actually makes, and it is ours to keep.
  var raw = JSON.stringify(verdict.presented_credentials ||
      verdict.presented_raw_data ||
                           verdict.presented_presentations || {});
  assert.ok(raw.indexOf(REQUESTED[0]) === -1,
    "the withheld claim (" + REQUESTED[0] +
        ") must not appear in what walt.id received. Its record: " +
    raw.slice(0, 400));
  assert.ok(raw.indexOf(REQUESTED[1]) !== -1,
    "while the claim that WAS disclosed (" + REQUESTED[1] +
        ") should. Its record: " + raw.slice(0, 400));

  // The wallet is the one that has to notice, and it is on step 3 already.
  var shortfall = await driver.executeScript(
    "return { answered: " +
        "document.getElementById('vp_answered').textContent.trim()," +
    "         sent: " +
        "document.getElementById('vp_sent_status').textContent.trim() };");
  assert.ok(/NOT fully answered/.test(shortfall.answered),
    "step 3 should say the request was not fully answered — no verifier is " +
        "going to say it for us. Got: " +
    shortfall.answered);
  ...
  assert.ok(/withheld/.test(shortfall.sent),
    "and the pane's own summary should not read as though nothing was " +
        "missing. Got: " + shortfall.sent);

  log.info("[negative 1] OK — " + REQUESTED[0] +
           " never reached walt.id, our step 3 reports the " +
           "shortfall, and walt.id itself said status=" +
               JSON.stringify(verdict.status) +
           " (it runs no DCQL-fulfilment policy — recorded here as an " +
               "interop finding).");
...
//
// Without this the suite proves nothing by walt.id's SUCCESSFUL: a verifier
// that accepted everything would pass every other assertion here. So take the
// exact bytes it just accepted and post them to a SECOND session. The Key
// Binding JWT carries the first session's nonce, which is what makes it a
// replay, and nonce-check is a policy walt.id really does run.
//
...
async function negativeReplay(held, accepted) {
  ...
  log.info("=== NEGATIVE 2: replaying an accepted presentation into a fresh " +
           "session ===");
  var session = await createVerificationSession({ vct: held.payload.vct });
  ...
  assert.ok(responseUri,
            "the second session's request should carry a response_uri.");
  assert.ok(freshNonce, "and a nonce of its own, which is what the replayed " +
  ...
  log.info("[negative 2] walt.id answered the replay with HTTP " +
           posted.status + ": " +
           String(posted.raw).replace(/\s+/g, " ").slice(0, 200));

  var verdict = await waltidVerdict(session.sessionId,
      "negative (replayed presentation)");
  var status = String(verdict.status || "");
  var refused = posted.status >= 400 || /FAIL|ERROR|REJECT/i.test(status) ||
      !!verdict.failure;
  assert.ok(refused,
    "walt.id must refuse a presentation bound to another session's nonce — " +
        "if it accepts this, its " +
    "SUCCESSFUL on the positive cases means nothing. HTTP " + posted.status +
        ", status=" + status +
    ", failure=" + JSON.stringify(verdict.failure || null).slice(0, 300));
  ...
  log.info("[negative 2] OK — walt.id refused the replay (HTTP " +
           posted.status + ", status=" + status +
           "), so its acceptance of the honest presentations is a " +
               "real verdict.");
...
async function test() {
  ...
  log.info("Starting Test run. verifier=" + verifierBase + ", issuer=" +
           issuerBase +
           ", wallet=" + baseUrl);
  ...
  assert.ok(live.status && live.status < 500,
    "walt.id's verifier should be reachable at " + verifierBase + " (HTTP " +
        live.status + " " +
  ...
  try {
    ...
    log.info("[browser] OK — no console errors across the walt.id " +
             "presentation flow.");
