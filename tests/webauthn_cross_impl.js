...
// takes DER natively and does no conversion at all. A mistake in one is
// therefore not mirrored in the other, which is the property
// `tests/bbs2023_cryptosuite.js` established for bbs-2023 and the reason the
// STS's verifier was written from the specification rather than by importing
// the client's.
//
...
// directory of its own, because unlike bbs2023.js it has a relative require);
// the sts/ submodule in a checkout; and the sibling development clone, for the
...
let sts;
try {
  sts = paths.requireSharedModule(STS_CANDIDATES,
      "the STS's WebAuthn verifier");
} catch (e) {
  if (/Cannot find module '\.\/helpers'/.test(e.message)) {
    log.error(
      "The STS's WebAuthn verifier at " + stsWhich +
          " still does `require('./helpers')`, so it " +
      "cannot be loaded outside the service — and it is loaded outside the " +
          "service here, and in " +
      "the tests image, where only that one file is staged. The fix is in " +
          "the module itself (a " +
      "silent-logger fallback when ./helpers is absent); staging the " +
...
const REG_CHALLENGE = Buffer.from(
  Uint8Array.from([0xde, 0xad, 0xbe,
                  0xef].concat(Array(28).fill(0x11)))).toString("base64url");
const AUTH_CHALLENGE = Buffer.from(
  Uint8Array.from([0xca, 0xfe, 0xba,
                  0xbe].concat(Array(28).fill(0x22)))).toString("base64url");
...
async function walletAssertion(vector, jwk, opts) {
  log.debug("Entering walletAssertion().");
  log.debug("Leaving walletAssertion().");
...
function stsAssertion(vector, jwk, opts) {
  log.debug("Entering stsAssertion().");
  log.debug("Leaving stsAssertion().");
...
async function test() {
  ...
  await check("both implementations read the same registration identically",
    ...
    ["es256_registration", "rs256_registration"].forEach(function (name) {
      ...
      assert.ok(theirs.ok, name +
                ": the STS should accept this registration; failing: " +
                theirs.failed.join("; "));
      assert.strictEqual(theirs.fmt, mine.fmt, name + ": attestation format");
      assert.strictEqual(theirs.aaguid, mine.aaguid, name + ": AAGUID");
      assert.strictEqual(theirs.credentialId, mine.credentialId, name +
                         ": credential ID");
      assert.strictEqual(theirs.signCount, mine.signCount, name +
                         ": sign count");
  ...
  await check("both accept the same valid assertions", async () => {
    ...
    for (const [label, reg, asr] of pairs) {
      ...
      const mine = await walletAssertion(asr, jwk,
          { requireUserVerification: true });
      const theirs = stsAssertion(asr, stripJwk(jwk),
          { requireUserVerification: true });
      assert.ok(mine.valid, label + ": the wallet should accept it; failing: " +
        mine.checks.filter(c => !c.ok).map(c => c.name).join("; "));
      assert.ok(theirs.ok, label + ": the STS should accept it; failing: " +
                theirs.failed.join("; "));
  ...
  await check("both reject the same tampered assertion, on the same check",
              async () => {
    ...
    sig[10] ^= 0xff;
    const tampered = Object.assign({}, asr,
    ...
    assert.deepStrictEqual(mineFailed.length, 1,
                           "the wallet should fail exactly one check: " +
      mineFailed.join("; "));
    assert.deepStrictEqual(theirs.failed.length, 1,
                           "the STS should fail exactly one check: " +
      theirs.failed.join("; "));
    assert.ok(/signature/.test(mineFailed[0]) &&
              /signature/.test(theirs.failed[0]),
  ...
  await check("both treat a UV-clear assertion the same way", async () => {
    ...
    const mineStrict = await walletAssertion(asr, jwk,
        { requireUserVerification: true });
    const theirsStrict = stsAssertion(asr, stripJwk(jwk),
        { requireUserVerification: true });
    ...
    assert.ok(mineStrict.signatureValid && theirsStrict.signatureValid,
      "and BOTH must still report the signature as valid — an implementation " +
          "that called this a " +
      "bad signature would send its operator after the wrong problem");
    assert.ok(/user verification/i.test(mineStrict.checks.filter(c =>
              !c.ok)[0].name) &&
  ...
  await check("both reject a challenge, origin and RP ID mismatch", async () =>
    ...
    const cases = [
      ["challenge", { challenge: Buffer.alloc(32, 9).toString("base64url") },
                     { expectedChallenge: Buffer.alloc(32,
                      9).toString("base64url") }, /challenge/],
      ["origin", { origin: "https://evil.example" },
       { expectedOrigin: "https://evil.example" }, /origin/],
      ["rpId", { rpId: "evil.example" }, { expectedRpId: "evil.example" },
       /rpIdHash/],
    ];
    for (const [label, mineOpts, theirsOpts, pattern] of cases) {
      ...
      assert.ok(m.some(n => pattern.test(n)) && theirs.failed.some(n =>
                pattern.test(n)),
        label + ": both must name it — wallet " + m.join("/") + ", STS " +
            theirs.failed.join("/"));
    }
...
program
  ...
  .description("The wallet's WebAuthn decoder and the STS's, over the same " +
      "real ceremonies.")
