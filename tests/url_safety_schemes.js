...
var urlSafety = paths.requireSharedModule(
  [__dirname + "/../client/src/url_safety.js", __dirname + "/url_safety.js"],
   "url_safety.js");
...
function dangerousSchemesAreRefused() {
  ...
  refused.forEach(function (value) {
    assert.throws(
      ...
      function (e) {
        // The message has to name the field and the refusal, because it is
        // shown to the user in a status line.
  ...
  log.info("[refuse] OK — " + refused.length +
           " dangerous or non-navigable URLs all refused.");
  log.debug("Leaving dangerousSchemesAreRefused().");
}

function emptyAndMalformedAreRefused() {
  log.debug("Entering emptyAndMalformedAreRefused().");
  log.info("[refuse] Empty and unparseable values must be refused.");
  const refused = ["", "   ", null, undefined, 42, {}, [], "http://",
      "not a url", "://missing-scheme"];
  refused.forEach(function (value) {
    assert.throws(function () { urlSafety.safeExternalUrl(value,
                  "The endpoint"); },
      "should have refused " + JSON.stringify(value));
  });
  log.info("[refuse] OK — " + refused.length +
           " empty or unparseable values all refused.");
  log.debug("Leaving emptyAndMalformedAreRefused().");
}


// --- what must be allowed ---------------------------------------------------
//
// A control that refuses real input is not a control, it is an outage. These
// are the URLs this debugger exists to send people to.
...
function realEndpointsAreAllowed() {
  ...
  allowed.forEach(function (value) {
    ...
    assert.ok(typeof out === "string" && out.length > 0,
              "should have returned a URL for " + value);
    assert.ok(/^https?:\/\//i.test(out),
              "the returned URL should be http(s): " + out);
...
function queryStringsSurviveIntact() {
  ...
  log.info("[allow] A multi-parameter query string must survive byte " +
           "for byte.");
  ...
  assert.ok(out.indexOf("&amp;") === -1,
    "the guard HTML-escaped an ampersand, which corrupts the query string: " +
        out);
  ["SAMLRequest=abc%2Bdef", "RelayState=xyz", "SigAlg=",
   "Signature=Zm9vYmFy"].forEach(function (part) {
    assert.ok(out.indexOf(part) >= 0, "lost query parameter " + part +
              " from " + out);
  });
  assert.strictEqual(out.split("&").length, 4, "the parameter count changed: " +
                     out);
...
function browserRelativeResolution() {
  ...
  log.info("[browser] With a window.location present, blanks must still " +
           "be refused.");
  const hadWindow = Object.prototype.hasOwnProperty.call(global, "window");
  const previous = global.window;
  global.window =
      { location: { href: "https://wallet.example.com/saml_request.html" } };
  try {
    ["", "   ", "\t\n"].forEach(function (blank) {
      assert.throws(function () { urlSafety.safeExternalUrl(blank,
                    "The endpoint"); },
        "a blank value must be refused even when it would resolve against " +
            "the page URL: " +
        JSON.stringify(blank));
    });

    // A dangerous scheme is still dangerous with a base present.
    assert.throws(function () { urlSafety.safeExternalUrl(
                  "javascript:alert(1)"); },
      "javascript: must be refused with a base present too");

    // A relative endpoint resolves against the page, which is a legitimate
    // thing for a same-origin path to do.
    assert.strictEqual(
    ...
    // Worth being explicit about the boundary: this is a SCHEME allowlist, not
    // a host allowlist, so a protocol-relative URL to another host is allowed.
    // That is correct here — a debugger exists to be pointed at arbitrary
    // identity providers, so restricting the host would break its purpose. The
    // property being defended is "no script execution / no document injection",
    // not "no navigation off-site".
    assert.strictEqual(
  ...
  log.info("[browser] OK — blanks refused, javascript: refused, relative " +
           "URLs resolve.");
...
function dompurifyWouldNotHaveHelped() {
  ...
  ["javascript:alert(1)", "java\tscript:alert(1)"].forEach(function (attack) {
    assert.strictEqual(DOMPurify.sanitize(attack), attack,
      "if DOMPurify ever starts refusing javascript: URLs, this note can be " +
          "revisited");
  ...
  assert.strictEqual(DOMPurify.sanitize(ordinary), ordinary,
    "an ordinary query string is left alone by DOMPurify — do not claim " +
        "otherwise");
  ...
  assert.ok(mangled.indexOf("<y>") === -1,
            "expected the <y> to be dropped, got: " + mangled);
  assert.strictEqual(urlSafety.safeExternalUrl(withMarkup).indexOf("&amp;"), -1,
    "url_safety must not entity-escape anything");
  log.info("[contrast] OK — DOMPurify returns `javascript:` URLs unchanged " +
           "(no protection) and " +
           "drops content from a URL containing markup; the scheme allowlist " +
               "does neither.");
  log.debug("Leaving dompurifyWouldNotHaveHelped().");
}
...
program
  .name("url_safety")
  .description("Verify the scheme allowlist applied before the app navigates " +
      "anywhere.")
