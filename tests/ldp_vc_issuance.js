...
// two. dc+sd-jwt and jwt_vc_json are both JWS-secured strings; this is a
// JSON-LD document with an EMBEDDED proof, and that is exactly why it can carry
// BBS at all — a BBS signature has no home in a JWS, because there is no BBS
// `alg` in JOSE.
//
...
var log = bunyan.createLogger({ name: "ldp_vc_issuance",
...
async function test() {
  ...
  assert.deepStrictEqual(entry.credential_signing_alg_values_supported,
                         ["bbs-2023"],
    "for ldp_vc this member holds a CRYPTOSUITE, not a JOSE alg — that is " +
        "the visible sign that the " +
    "credential is secured by an embedded Data Integrity proof rather than " +
        "by a JWS. Got: " +
  ...
  assert.strictEqual(cred.proof.cryptosuite, "bbs-2023",
                     "and names the cryptosuite.");
  assert.ok(/^u/.test(cred.proof.proofValue),
            "proofValue is multibase base64url ('u').");
  ...
  log.info("[issue] OK — a " + [].concat(cred.type).join("/") +
           " with an embedded bbs-2023 proof.");
  ...
  assert.ok(km && km.publicKeyMultibase,
    "the verificationMethod should resolve to a key. Got: " +
        JSON.stringify(km).slice(0, 160));
  assert.strictEqual(km.cryptosuite, "bbs-2023",
                     "and name the cryptosuite it is for.");
  ...
  assert.strictEqual(pk.length, 96,
    "a BLS12-381 G2 public key is 96 compressed bytes. A short key here " +
        "means the multibase prefix " +
    "was stripped twice — a decoder that also strips it, plus a caller that " +
        "strips it, eats a data " +
    "character whenever the payload starts with 'u'. That is intermittent: " +
        "it passes for most keys.");

  const verdict = await stsSuite.verifyBase(cred, pk);
  assert.strictEqual(verdict.ok, true,
    "the base proof must verify against the key the credential points at. If " +
        "this fails the issuer is " +
    "publishing a key it did not sign with.");
  assert.ok(verdict.statements.length > 4,
    "the credential should canonicalize to several statements; got " +
        verdict.statements.length);
  log.info("[verify] OK — the base proof verifies over " +
           verdict.statements.length + " statements.");
  ...
  assert.ok(held.responseBody,
      "the mint helper should hand back the Credential Response it received.");
  const stored = wallet.extractCredential(held.responseBody);
  assert.strictEqual(typeof stored, "string",
    "the wallet stores credentials as strings — localStorage takes a string, " +
        "and an object reaches it " +
    "as the literal \"[object Object]\". An ldp_vc must therefore be " +
        "serialized, not passed through.");
  assert.ok(stored,
    "extractCredential() returned nothing for an ldp_vc Credential Response. " +
        "This is the step 2 failure: " +
    "the issuer answered 200 with a credential and the wallet reported \"the " +
        "response carries no " +
    "credential\". Response was: " + JSON.stringify(held.responseBody).slice(0,
        300));
  ...
  assert.ok(!back.credential,
    "what was stored is the {credential: …} envelope, not the credential " +
        "inside it.");
  assert.deepStrictEqual(back, cred,
      "and should round-trip to exactly what the issuer sent.");
  ...
  // allCredentials() passed the object through while extractCredential() did
  // not.
  ...
  assert.strictEqual(all[0], stored,
    "allCredentials() and extractCredential() must produce the SAME " +
        "representation of the same " +
    "credential — step 2 stores the first under CREDENTIAL and the array " +
        "under CREDENTIALS, and step 3 " +
    "reads both.");
  log.info("[wallet] OK — the response reads as a " + stored.length +
           "-character string.");
  ...
  assert.strictEqual(wallet.extractCredential({ transaction_id: "t-1" }), "",
    "a deferred response carries no credential, and step 2 routes on exactly " +
        "that emptiness — if this " +
    "returns a value, the deferred pane never opens.");
  assert.strictEqual(wallet.extractCredential({ credentials: [] }), "",
                     "nor does an empty array.");
  assert.strictEqual(wallet.extractCredential(null), "", "nor a missing body.");
  log.info("[wallet] OK — and still reports nothing for a deferred/empty " +
           "response.");
  ...
  assert.strictEqual(tamperedVerdict.ok, false,
    "changing a claim must break the base proof — the control below shows " +
        "the check is not simply " +
    "always-false.");
  const control = await stsSuite.verifyBase(cred, pk);
  assert.strictEqual(control.ok, true,
                     "the control: the untouched credential still verifies.");
  log.info("[tamper] OK — an edited claim is refused; the control still " +
           "verifies.");
