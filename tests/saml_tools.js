...
function isHidden(driver, id) {
  log.debug("Entering isHidden().");
  log.debug("Leaving isHidden().");
...
function isOptionHidden(driver, selectId, value) {
  log.debug("Entering isOptionHidden().");
  log.debug("Leaving isOptionHidden().");
  return driver.executeScript(
...
async function testDefaults(driver) {
  ...
  assert.ok(xml.indexOf('<saml:Issuer>' + issuer + '</saml:Issuer>') !== -1,
    "The default Issuer (the debugger URL) is not in the assertion. Issuer=" +
        issuer);
  assert.ok(xml.indexOf('Version="2.0"') !== -1,
            "The page does not default to SAML 2.0.");
  ...
  assert.ok(xml.indexOf('<saml:NameID') !== -1,
            "No <saml:NameID> in the assertion.");
  assert.ok(xml.indexOf('<saml:AuthnStatement') !== -1,
            "No <saml:AuthnStatement> in the assertion.");
  assert.ok(xml.indexOf('AttributeStatement') === -1,
    "An <AttributeStatement> was emitted before any attribute was added.");
  log.info("[defaults] OK — SAML 2.0 assertion, Issuer=" + issuer +
           ", instants populated.");
...
async function testOptionalElements(driver) {
  ...
  // Each toggle must add and remove its element. Checkbox id -> marker in the
  // XML.
  var toggles = [
  ...
  for (var i = 0; i < toggles.length; i++) {
    ...
    await waitForValue(driver, 'sa_assertion',
                       function (v) { return v.indexOf(marker) !== -1; },
  ...
  // Turning the Conditions off drops the whole block (and its children with
  // it).
  await click(driver, By.id('sa_opt_conditions'));
  await waitForValue(driver, 'sa_assertion',
      function (v) { return v.indexOf('<saml:Conditions') === -1; },
...
async function testCustomAttribute(driver) {
  ...
  assert.ok(await isHidden(driver, 'sa_attr_group'),
    "The Add Custom SAML Attribute fields should be hidden until the " +
        "checkbox is ticked.");
  await click(driver, By.id('sa_opt_attrs'));
  assert.ok(!(await isHidden(driver, 'sa_attr_group')),
    "Ticking the attribute checkbox should reveal the Add Custom SAML " +
        "Attribute fields.");
  ...
  assert.ok(xml.indexOf(
      'NameFormat="urn:oasis:names:tc:SAML:2.0:attrname-format:uri"') !== -1,
  ...
  assert.ok(xml.indexOf('xmlns:xs="http://www.w3.org/2001/XMLSchema"') !== -1 &&
    ...
    "The xs/xsi namespaces used by xsi:type are not declared on the " +
        "assertion.");
  ...
  assert.strictEqual(rows.length, 1,
                     "The attribute table should list exactly one attribute.");
  ...
  await click(driver, onclickBtn('addAttribute'));
  xml = await waitForValue(driver, 'sa_assertion',
      function (v) { return v.indexOf('employeeNumber') !== -1; },
    "the second attribute was not added.");
  assert.ok(xml.indexOf('xsi:type="xs:integer"') !== -1,
            "The integer value type was not emitted.");
  ...
  assert.strictEqual(removes.length, 2,
                     "Expected a Remove button per attribute row.");
  await driver.executeScript("arguments[0].scrollIntoView({ block: " +
                             "'center' });", removes[1]);
...
async function testDependentFieldCollapse(driver) {
  ...
  for (var i = 0; i < deps.length; i++) {
    ...
    assert.ok(!(await isHidden(driver, group)),
      "[" + box + "] " + group +
          " should be visible while the box is checked.");
    await click(driver, By.id(box));                     // turn it off
    assert.ok(await isHidden(driver, group),
      "[" + box + "] " + group +
          " should be hidden while the box is unchecked.");
    if (wasOn) await click(driver, By.id(box));          // restore
  }
  log.info("[dependencies] OK — " + deps.length +
           " checkbox/field-group pairs collapse and expand.");
...
async function testTooltips(driver) {
  ...
  var missing = await driver.executeScript(
    "var bad = [];" +
    "var els = document.querySelectorAll('.saml-pane input, .saml-pane " +
        "select, .saml-pane textarea, .saml-pane label');" +
    "for (var i = 0; i < els.length; i++) {" +
    "  var e = els[i];" +
    "  if (e.type === 'button' || e.type === 'submit') { if (!e.title) " +
        "bad.push(e.value || e.id); continue; }" +
    "  if (!e.title) bad.push(e.id || e.htmlFor || " +
...
async function testPrettyPrint(driver) {
  ...
  assert.strictEqual(firstField, 'sa_assertion',
    "The Generated Assertion field should be the first field in pane " +
        "1. Found: " + firstField);
  ...
  assert.ok(flat.indexOf('\n') === -1,
            "The assertion was not flattened for the test.");
  await click(driver, onclickBtn('prettyPrintAssertion'));
  var pretty = await waitForValue(driver, 'sa_assertion',
...
async function signAndVerify(driver, label, expect) {
  ...
  await driver.executeScript(
      "document.getElementById('sa_signed_assertion').value = '';");
  ...
  assert.ok(signed.indexOf('<ds:Signature') !== -1, "[" + label +
            "] no <ds:Signature> was produced.");
  ...
  if (expect.signatureLast) {
    assert.ok(/<\/ds:Signature><\/saml:Assertion>\s*$/.test(signed.trim()),
      "[" + label +
          "] the <ds:Signature> must be the assertion's last child:\n" +
          signed.slice(-200));
  } else {
    var afterIssuer = signed.indexOf('</saml:Issuer>');
    var sigAt = signed.indexOf('<ds:Signature');
    assert.ok(sigAt > afterIssuer && signed.slice(afterIssuer +
              '</saml:Issuer>'.length, sigAt).trim() === '',
      "[" + label +
          "] the <ds:Signature> must directly follow <saml:Issuer>:\n" +
          signed.slice(0, 600));
  }

  await driver.executeScript(
      "document.getElementById('sa_verify_output').value = '';");
  await click(driver, onclickBtn('verifySignature'));
  var out = await waitForValue(driver, 'sa_verify_output',
    function (v) { return v.length > 0; }, "[" + label +
              "] verification produced no output.");
  assert.ok(out.indexOf('VALID') === 0, "[" + label +
  ...
  var bad = await waitForValue(driver, 'sa_verify_output',
    function (v) { return v.length > 0; }, "[" + label +
              "] tamper verification produced no output.");
...
async function testAutoUpdate(driver) {
  ...
  assert.strictEqual(await getValue(driver, 'sa_assertion'), signed,
    "Clicking Sign Assertion should put the signed assertion in the " +
        "Generated Assertion box.");
  var beforeEnc = await getValue(driver, 'sa_encrypted');
  assert.ok(beforeEnc.length > 0,
            "expected a ciphertext from the previous step.");
  ...
  assert.notStrictEqual(resigned, signed,
                        "the signature did not change with the assertion.");
  ...
  assert.ok(out.indexOf('VALID') === 0,
            "the recomputed signature does not verify:\n" + out);
  ...
  assert.strictEqual(dec, resigned,
    "the recomputed ciphertext does not carry the recomputed signed " +
        "assertion.");

  // A pane 2 setting change re-signs as well.
  await selectValue(driver, 'sa_sig_alg',
                    'http://www.w3.org/2001/04/xmldsig-more#rsa-sha512');
  await waitForValue(driver, 'sa_signed_assertion',
                     function (v) { return v.indexOf('rsa-sha512') !== -1; },
    "changing the signature algorithm did not re-sign the assertion.");
  await selectValue(driver, 'sa_sig_alg',
                    'http://www.w3.org/2001/04/xmldsig-more#rsa-sha256');
  await waitForValue(driver, 'sa_signed_assertion',
                     function (v) { return v.indexOf('rsa-sha256') !== -1; },
    "restoring the signature algorithm did not re-sign the assertion.");
  log.info("[live updates] OK — edits re-sign and re-encrypt; pane 1 tracks " +
           "the signed form.");
...
async function testReset(driver) {
  ...
  log.info("=== Reset ===");
  assert.strictEqual((await driver.findElements(By.xpath(
                     '//input[@value="Rebuild"]'))).length, 0,
  ...
  await selectValue(driver, 'sa_enc_data_alg',
                    'http://www.w3.org/2001/04/xmlenc#aes128-cbc');
  await waitForValue(driver, 'sa_assertion',
      function (v) { return v.indexOf('reset.example.com') !== -1; },
  ...
  var expected = [
    ...
    ['sa_private_key', ''], ['sa_public_key', ''], ['sa_enc_cert', ''],
     ['sa_enc_private_key', ''],
    ['sa_signed_assertion', ''], ['sa_encrypted', ''], ['sa_verify_output', ''],
     ['sa_dec_output', ''],
  ...
  assert.strictEqual(await driver.findElement(By.id('sa_opt_attrs'))
                     .isSelected(), false,
    "Reset should leave the custom-attribute checkbox unchecked.");
  assert.strictEqual(await driver.findElement(By.id('sa_opt_conditions'))
                     .isSelected(), true,
  ...
  assert.strictEqual(rows.length, 1,
                     "Reset should drop every custom attribute.");

  var xml = await getValue(driver, 'sa_assertion');
  assert.ok(xml.indexOf('Version="2.0"') !== -1,
            "Reset should rebuild a SAML 2.0 assertion.");
  assert.ok(xml.indexOf('<ds:Signature') === -1,
            "Reset should leave the assertion unsigned.");
  assert.ok(xml.indexOf('AttributeStatement') === -1,
            "Reset should drop the AttributeStatement.");
...
async function setCheckboxes(driver, state) {
  log.debug("Entering setCheckboxes().");
  await driver.executeScript(
    "var s = arguments[0], last = null;" +
    "for (var id in s) { var e = document.getElementById(id); if (!e) " +
        "continue;" +
  ...
  log.debug("Leaving setCheckboxes().");
}
var ALL_ON = {
  sa_opt_subject: true, sa_opt_subjconf: true, sa_opt_conditions: true,
      sa_opt_audience: true,
  sa_opt_onetimeuse: true, sa_opt_proxy: true, sa_opt_advice: true,
      sa_opt_authn: true,
...
async function visibleOptions(driver, id) {
  log.debug("Entering visibleOptions().");
  log.debug("Leaving visibleOptions().");
  return driver.executeScript(
    "var s = document.getElementById(arguments[0]); var out = [];" +
    "for (var i = 0; i < s.options.length; i++) { if (!s.options[i].hidden) " +
        "out.push(s.options[i].value); }" +
    "return out;", id);
}

async function complianceIsClean(driver, context) {
  log.debug("Entering complianceIsClean().");
  await driver.executeScript(
      "document.getElementById('sa_compliance_output').value = '';");
  await click(driver, onclickBtn('checkCompliance'));
  var out = await waitForValue(driver, 'sa_compliance_output',
      function (v) { return v.length > 0; },
    context + ": the compliance check produced no output.");
  assert.ok(out.indexOf('no failures') !== -1, context +
            ": assertion is not compliant:\n" + out);
  log.debug("Leaving complianceIsClean().");
}
...
async function testAllSelectOptions(driver, v) {
  ...
  var v2 = v === '2.0';
  var cmPrefix = v2 ?
      'urn:oasis:names:tc:SAML:2.0:cm:' : 'urn:oasis:names:tc:SAML:1.0:cm:';
  var cases = [
    { id: 'sa_nameid_format', expect: function (xml, o) {
      log.debug("Entering expect().");
      log.debug("Leaving expect().");
      return xml.indexOf('Format="' + o + '"') !== -1;
    } },
    { id: 'sa_confirm_method', expect: function (xml, o) {
      log.debug("Entering expect().");
      log.debug("Leaving expect().");
      return xml.indexOf(cmPrefix + o) !== -1;
    } },
    { id: 'sa_authz_decision', expect: function (xml, o) {
      log.debug("Entering expect().");
      log.debug("Leaving expect().");
      return xml.indexOf('Decision="' + o + '"') !== -1;
    } },
    v2
      ? { id: 'sa_authn_context', expect: function (xml, o) {
        log.debug("Entering expect().");
        log.debug("Leaving expect().");
        return xml.indexOf('<saml:AuthnContextClassRef>' + o + '<') !== -1;
      } }
      : { id: 'sa_authn_method', expect: function (xml, o) {
        log.debug("Entering expect().");
        log.debug("Leaving expect().");
        return xml.indexOf('AuthenticationMethod="' + o + '"') !== -1;
      } },
  ];
  var total = 0;
  for (var c = 0; c < cases.length; c++) {
    var opts = await visibleOptions(driver, cases[c].id);
    assert.ok(opts.length > 0, "[" + v + "] " + cases[c].id +
              " offers no selectable option.");
    for (var i = 0; i < opts.length; i++) {
      var o = opts[i];
      await selectValue(driver, cases[c].id, o);
      var expect = cases[c].expect;
      await waitForValue(driver, 'sa_assertion',
                         function (xml) { return expect(xml, o); },
        "[" + v + "] " + cases[c].id + "=" + o +
            " did not reach the assertion.");
      await complianceIsClean(driver, "[" + v + "] " + cases[c].id + "=" + o);
      total++;
    }
  }
  log.info("[" + v + "] OK — " + total +
           " select option(s) exercised, each reflected and compliant.");
...
async function testAllTextFields(driver, v) {
  ...
  var fields = [
    { id: 'sa_issuer', value: 'https://sweep-' + v + '.example.com/issuer',
      expect: function (x, s) {
        log.debug("Entering expect().");
        log.debug("Leaving expect().");
        return v2 ? x.indexOf('<saml:Issuer>' + s +
            '</saml:Issuer>') !== -1 : x.indexOf('Issuer="' + s + '"') !== -1;
      } },
    { id: 'sa_nameid_value', value: 'sweep-user@example.com',
      expect: function (x, s) {
        log.debug("Entering expect().");
        log.debug("Leaving expect().");
        return x.indexOf('>' + s + '</saml:Name') !== -1;
      } },
    { id: 'sa_nameid_qualifier', value: 'urn:sweep:name-qualifier',
      expect: function (x, s) {
        log.debug("Entering expect().");
        log.debug("Leaving expect().");
        return x.indexOf('NameQualifier="' + s + '"') !== -1;
      } },
    { id: 'sa_audience', value: 'https://sweep.example.com/sp',
      expect: function (x, s) {
        log.debug("Entering expect().");
        log.debug("Leaving expect().");
        return x.indexOf('<saml:Audience>' + s + '</saml:Audience>') !== -1;
      } },
    ...
    { id: 'sa_locality_address', value: '198.51.100.7',
      expect: function (x, s) {
        log.debug("Entering expect().");
        log.debug("Leaving expect().");
        return x.indexOf((v2 ? 'Address="' : 'IPAddress="') + s + '"') !== -1;
      } },
    { id: 'sa_locality_dns', value: 'sweep.example.com',
      expect: function (x, s) {
        log.debug("Entering expect().");
        log.debug("Leaving expect().");
        return x.indexOf((v2 ? 'DNSName="' : 'DNSAddress="') + s + '"') !== -1;
      } },
    { id: 'sa_authz_resource', value: 'https://sweep.example.com/protected',
      expect: function (x, s) {
        log.debug("Entering expect().");
        log.debug("Leaving expect().");
        return x.indexOf('Resource="' + s + '"') !== -1;
      } },
    { id: 'sa_authz_action', value: 'Execute',
      expect: function (x, s) {
        log.debug("Entering expect().");
        log.debug("Leaving expect().");
        return x.indexOf('>' + s + '</saml:Action>') !== -1;
      } },
    { id: 'sa_authz_action_ns',
     value: 'urn:oasis:names:tc:SAML:1.0:action:rwedc-negation',
      expect: function (x, s) {
        log.debug("Entering expect().");
        log.debug("Leaving expect().");
        return x.indexOf('Namespace="' + s + '"') !== -1;
      } },
    { id: 'sa_id', value: '_sweepassertionid' + v.replace('.', ''),
      expect: function (x, s) {
        log.debug("Entering expect().");
        log.debug("Leaving expect().");
        return x.indexOf((v2 ? 'ID="' : 'AssertionID="') + s + '"') !== -1;
      } },
    { id: 'sa_issue_instant', value: '2026-03-04T05:06:07Z',
      expect: function (x, s) {
        log.debug("Entering expect().");
        log.debug("Leaving expect().");
        return x.indexOf('IssueInstant="' + s + '"') !== -1;
      } },
    { id: 'sa_not_before', value: '2026-03-04T05:00:00Z',
      expect: function (x, s) {
        log.debug("Entering expect().");
        log.debug("Leaving expect().");
        return x.indexOf('NotBefore="' + s + '"') !== -1;
      } },
    { id: 'sa_not_on_or_after', value: '2026-03-04T06:00:00Z',
      expect: function (x, s) {
        log.debug("Entering expect().");
        log.debug("Leaving expect().");
        return x.indexOf('NotOnOrAfter="' + s + '"') !== -1;
      } },
    { id: 'sa_authn_instant', value: '2026-03-04T05:06:07Z',
      expect: function (x, s) {
        log.debug("Entering expect().");
        log.debug("Leaving expect().");
        return x.indexOf((v2 ? 'AuthnInstant="' : 'AuthenticationInstant="') +
                         s + '"') !== -1;
      } },
  ];
    ...
    fields = fields.concat([
      ...
      { id: 'sa_session_index', value: '_sweepsession1',
        expect: function (x, s) {
          log.debug("Entering expect().");
          log.debug("Leaving expect().");
          return x.indexOf('SessionIndex="' + s + '"') !== -1;
        } },
      { id: 'sa_session_notonorafter', value: '2026-03-04T09:06:07Z',
        expect: function (x, s) {
          log.debug("Entering expect().");
          log.debug("Leaving expect().");
          return x.indexOf('SessionNotOnOrAfter="' + s + '"') !== -1;
        } },
      { id: 'sa_proxy_count', value: '3',
        expect: function (x, s) {
          log.debug("Entering expect().");
          log.debug("Leaving expect().");
          return x.indexOf('Count="' + s + '"') !== -1;
        } },
    ]);
  }
  ...
  for (var i = 0; i < fields.length; i++) {
    ...
    await waitForValue(driver, 'sa_assertion',
                       function (xml) { return expect(xml, value); },
      "[" + v + "] " + f.id + '="' + f.value +
          '" did not reach the assertion.');
  }
  ...
  var after = await waitForValue(driver, 'sa_not_on_or_after',
      function (x) { return x !== before && x.length > 0; },
  ...
  var newId = await waitForValue(driver, 'sa_id',
      function (x) { return x !== oldId; },
    "[" + v + "] New ID did not change the assertion identifier.");
  assert.ok(newId.indexOf('_') === 0, "[" + v +
            "] a generated ID must be an NCName: " + newId);
  await waitForValue(driver, 'sa_assertion',
                     function (x) { return x.indexOf(newId) !== -1; },
    "[" + v + "] the new assertion ID did not reach the assertion.");
  await complianceIsClean(driver, "[" + v +
                          "] after the timestamp/ID controls");
  log.info("[" + v + "] OK — " + fields.length +
           " text field(s) plus the timestamp and ID controls.");
...
async function testAllAttributeTypes(driver, v) {
  ...
  for (var i = 0; i < types.length; i++) {
    ...
    var xml = await waitForValue(driver, 'sa_assertion',
        function (x) { return x.indexOf(name) !== -1; },
    ...
    if (v2) {
      ...
      assert.ok(xml.indexOf(
        ...
        "[" + v + "] wrong NameFormat for " + (prefix ?
            'a prefixed' : 'an unprefixed') + " attribute.");
    } else {
      assert.ok(xml.indexOf('AttributeName="' + name + '"') !== -1,
        "[" + v + "] SAML 1.x should use AttributeName for " + t + ".");
      assert.ok(xml.indexOf('AttributeNamespace="' + (prefix ||
                'urn:oasis:names:tc:SAML:1.0:assertion') + '"') !== -1,
    ...
    if (t === 'unspecified') {
      ...
      assert.ok(xml.indexOf('xsi:type="xs:' + t + '">' + value +
  ...
  await setInput(driver, 'sa_attr_value', ATTR_VALUE);
  await click(driver, onclickBtn('addAttribute'));
  log.info("[" + v + "] OK — " + types.length +
           " attribute value type(s), prefixed and unprefixed.");
...
var POWER_SET_SCRIPT = [
  ...
  "var ids = {subject:'sa_opt_subject', subjconf:'sa_opt_subjconf', " +
      "conditions:'sa_opt_conditions',",
  "           audience:'sa_opt_audience', onetimeuse:'sa_opt_onetimeuse', " +
      "proxy:'sa_opt_proxy',",
  "           advice:'sa_opt_advice', authn:'sa_opt_authn', " +
      "locality:'sa_opt_locality', authz:'sa_opt_authz'};",
  "var v2 = version === '2.0';",
  ...
  "  document.getElementById(ids.conditions).dispatchEvent(new " +
      "Event('change', { bubbles: true }));",
  "  states++;",
  "  var xml = document.getElementById('sa_assertion').value;",
  "  var label = version + ' [' + keys.filter(function (k) { return on[k]; " +
      "}).join(',') + ']';",
  "",
  "  var doc = parser.parseFromString(xml, 'application/xml');",
  "  if (doc.getElementsByTagName('parsererror').length) { " +
      "problems.push(label + ': not well-formed XML'); continue; }",
  "",
  ...
  "      problems.push(label + ': ' + markers[k] + ' is ' + (present ? " +
      "'present' : 'absent') +",
  "                    ' but should be ' + (expected[k] ? 'present' : " +
      "'absent'));",
  "    }",
  "  }",
  "",
  ...
  "    problems.push(label + ': compliance says ' + (clean ? 'clean' : " +
      "'failing') +",
  "                  ' but the combination is ' + (shouldBeClean ? 'valid' : " +
      "'invalid') + '\\n' + report);",
  "  }",
  "  if (problems.length > 12) { problems.push('... stopping after 12 " +
      "problems'); break; }",
  "}",
...
async function testOptionalElementPowerSet(driver, v) {
  ...
  var markers = {
    ...
    audience: v2 ?
        '<saml:AudienceRestriction>' : '<saml:AudienceRestrictionCondition>',
...
async function testSaml1x(driver, minor) {
  ...
  assert.ok(xml.indexOf(
            'xmlns:saml="urn:oasis:names:tc:SAML:1.0:assertion"') !== -1,
    "[" + label + "] SAML 1.x must use the 1.0 assertion namespace.");
  assert.ok(xml.indexOf('MajorVersion="1"') !== -1, "[" + label +
            "] MajorVersion is missing.");
  assert.ok(xml.indexOf('AssertionID="_') !== -1, "[" + label +
            "] SAML 1.x identifies the assertion by AssertionID.");
  assert.ok(xml.indexOf('<saml:Issuer>') === -1 && /\sIssuer="[^"]+"/.test(xml),
    "[" + label +
        "] SAML 1.x carries the Issuer as an attribute, not an element.");
  ...
  assert.ok(xml.indexOf('<saml:Subject>') > xml.indexOf(
  ...
  assert.ok(xml.indexOf('AttributeName="' + ATTR_NAME + '"') !== -1 &&
    ...
    "[" + label +
        "] the URI prefix should become AttributeNamespace in SAML 1.x:\n" +
        xml);
  ...
  // DoNotCacheCondition arrived in 1.1; it must never appear in a 1.0
  // assertion.
  var otuHidden = await isHidden(driver, 'sa_row_onetimeuse');
  if (minor === '0') {
    assert.ok(otuHidden, "[" + label + "] the OneTimeUse/DoNotCacheCondition " +
              "row must be hidden for SAML 1.0.");
  } else {
    assert.ok(!otuHidden, "[" + label +
              "] DoNotCacheCondition should be offered for SAML 1.1.");
    ...
    await waitForValue(driver, 'sa_assertion',
        function (v) { return v.indexOf('DoNotCacheCondition') === -1; },
      "[" + label + "] <DoNotCacheCondition> should be absent while the box " +
          "is unchecked.");
    await click(driver, By.id('sa_opt_onetimeuse'));
    await waitForValue(driver, 'sa_assertion', function (v) { return v.indexOf(
                       '<saml:DoNotCacheCondition/>') !== -1; },
  ...
  assert.ok(await isHidden(driver, 'sa_row_confirm_data'),
    "[" + label +
        "] the SubjectConfirmationData fields must be hidden (SAML 2.0 only).");
  assert.ok(await isHidden(driver, 'sa_row_enc_wrap'),
    "[" + label + "] the EncryptedAssertion wrapper option must be hidden in " +
        "pane 3 (SAML 2.0 only).");
  // Down to individual <select> options.
  assert.strictEqual(await isOptionHidden(driver, 'sa_confirm_method',
                     'artifact'), false,
    "[" + label +
        "] the artifact confirmation method should be offered for SAML 1.x.");
  assert.strictEqual(
    await isOptionHidden(driver, 'sa_nameid_format',
...
async function testCryptoOnce(driver) {
  ...
  log.info("=== Panes #2 and #3 — key generation, sign/validate, " +
           "encrypt/decrypt ===");

  await click(driver, onclickBtn('generateKeys'));
  await waitForValue(driver, 'sa_private_key',
                     function (v) { return v.indexOf('BEGIN') !== -1; },
  ...
  await waitForValue(driver, 'sa_enc_private_key',
                     function (v) { return v.indexOf('BEGIN') !== -1; },
  ...
  assert.notStrictEqual(encCert, signCert,
    "pane 3 should generate its own recipient certificate, not reuse the " +
        "signing one.");
...
async function testNoConsoleErrors(driver) {
  ...
  try {
    ...
    log.info("[console] SKIP — the browser log is unavailable here: " +
             e.message);
...
async function testToolsPane(driver) {
  ...
  await click(driver,
              By.xpath("//legend[contains(@onclick, \"pane_tools_body\")]"));
  ...
  assert.ok(ret.indexOf('/saml_request.html') !== -1,
    "The assertion page should return to the SAML Test Tools page. Found: " +
        ret);
...
async function testResponseToolsPane(driver) {
  ...
  var text = await link.getText();
  assert.ok(text.indexOf('SAML Assertion Tool') !== -1,
    "The SAML Response Tools pane does not link to the SAML Assertion " +
        "Tool. Found: " + text);
  var cert = await driver.findElements(By.css('#pane_tools_body ' +
      'a[href^="/saml_cert.html"]'));
  assert.strictEqual(cert.length, 1, "The SAML Response Tools pane should " +
                     "link to the certificate details page.");
  await driver.executeScript("arguments[0].scrollIntoView({ block: " +
                             "'center' });", link);
  await link.click();
  await driver.wait(until.urlContains('saml_tools.html'), waitTime);
...
async function samlAssertionActivities(driver) {
  ...
  assert.strictEqual(await isOptionHidden(driver, 'sa_confirm_method',
                     'artifact'), true,
    "The artifact confirmation method is SAML 1.x only and should be " +
        "hidden for 2.0.");
  assert.strictEqual(
    await isOptionHidden(driver, 'sa_nameid_format',
                         'urn:oasis:names:tc:SAML:2.0:nameid-format:transient'),
  ...
  await waitForValue(driver, 'sa_assertion',
                     function (x) { return x.indexOf('Version="2.0"') !== -1; },
...
program
  .name('saml_tools')
  .description("Run SAML Assertion Tool UI test (compose 1.0/1.1/2.0, sign, " +
      "encrypt).")
