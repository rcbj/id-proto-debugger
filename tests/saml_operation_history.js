...
// the dispatch aims at a URL on the test server, and the resolution is driven
// by handing the SAML Response page a crafted SAMLResponse. The full round-trip
...
async function historyRows(driver) {
  ...
  return driver.executeScript(
    "var rows = document.querySelectorAll('#saml_operation_history " +
        "tbody tr');" +
    "var out = [];" +
    "for (var i = 0; i < rows.length; i++) {" +
    "  var c = rows[i].getElementsByTagName('td');" +
    "  out.push({ n: c[0].textContent.trim(), time: c[1].textContent.trim()," +
    "             operation: c[2].textContent.trim(), binding: " +
        "c[3].textContent.trim()," +
    "             version: c[4].textContent.trim(), sp: " +
        "c[5].textContent.trim()," +
    "             idp: c[6].textContent.trim(), result: " +
...
// raw (POST-binding) message when it is not deflated, so plain base64 is
// enough.
function b64(xml) {
...
const RESP_OPEN =
  ...
  ' xmlns:saml="urn:oasis:names:tc:SAML:2.0:assertion" ID="_hist1" ' +
      'Version="2.0"' +
...
async function operationHistoryActivities(driver) {
  ...
  var empty = await driver.findElement(By.css('#saml_operation_history ' +
  ...
  var order = await driver.executeScript(
    "var t = document.getElementById('pane_tools'), h = " +
        "document.getElementById('pane_history');" +
    "return { after: !!(t.compareDocumentPosition(h) & " +
        "Node.DOCUMENT_POSITION_FOLLOWING)," +
    "         inCols: !!h.closest('.saml-cols') };");
  assert.ok(order.after,
            "The Operations History pane must come after the Tools pane.");
  assert.ok(!order.inCols,
  ...
  rows = await waitForRows(driver, 2,
      "the Single Logout attempt was not recorded.");
  assert.strictEqual(rows[0].operation, 'Single Logout',
                     "wrong operation recorded: " + rows[0].operation);
  assert.ok(rows[0].result.indexOf('Failure') === 0,
            "expected a failure, got: " + rows[0].result);
  assert.ok(rows[0].binding === 'HTTP-Redirect' ||
            rows[0].binding === 'HTTP-POST',
    "Single Logout should record the binding it would use: " + rows[0].binding);

  // ---- Failure: metadata load that cannot resolve -------------------------
  await setInput(driver, 'saml_metadata_url', baseUrl +
                 '/no-such-metadata.xml');
  await click(driver, btn('loadMetadata'));
  rows = await waitForRows(driver, 3,
      "the metadata load failure was not recorded.");
  assert.strictEqual(rows[0].operation, 'Load IdP Metadata',
                     "wrong operation recorded: " + rows[0].operation);
  assert.ok(rows[0].result.indexOf('Failure') === 0,
            "expected a failure, got: " + rows[0].result);
  assert.strictEqual(rows[0].binding, '—',
                     "a metadata load has no protocol binding: " +
                     rows[0].binding);
  ...
  if (await signBox.isSelected()) await click(driver,
      By.id('saml_sign_request'));
  ...
  await openPage(driver);
  rows = await waitForRows(driver, 4,
      "the dispatched AuthnRequest was not recorded.");
  var sent = rows[0];
  assert.strictEqual(sent.operation, 'Send AuthnRequest', "wrong operation: " +
                     sent.operation);
  assert.strictEqual(sent.binding, 'HTTP-Redirect', "wrong binding: " +
                     sent.binding);
  ...
  assert.ok(sent.result.indexOf('Sent') === 0,
    "a dispatched request must be recorded as Sent, not as a success: " +
        sent.result);
  assert.ok(sent.result.indexOf(ssoUrl) !== -1,
            "the destination should be noted: " + sent.result);
  assert.strictEqual(sent.resultClass, 'saml-pending',
                     "a pending dispatch should be styled as pending.");
  assert.strictEqual(sent.n, '4',
                     "entries should be numbered in the order they happened: " +
                     sent.n);
  log.info("[history] OK — the dispatch survived the navigation, recorded as " +
           "Sent (not Success).");
  ...
  await openPage(driver);
  rows = await waitForRows(driver, 4,
      "the history disappeared after the response.");
  var denied = rows[0];
  assert.ok(denied.result.indexOf('Failure') === 0,
    "a refused AuthnRequest must be recorded as a Failure, got: " +
        denied.result);
  assert.ok(denied.result.indexOf('Responder') !== -1 &&
            denied.result.indexOf('RequestDenied') !== -1,
  ...
  assert.strictEqual(denied.resultClass, 'saml-bad',
                     "a refusal should be styled as a failure.");
  assert.strictEqual(rows.length, 4,
                     "resolving must update the entry, not append one.");
  log.info("[resolve] OK — a refused request is recorded as a Failure with " +
           "the IdP's status.");
  ...
  assert.ok(rows[0].result.indexOf('Success') === 0,
    "a Success status should resolve the dispatch as a success: " +
        rows[0].result);
  assert.strictEqual(rows[0].resultClass, 'saml-ok',
                     "a success should be styled as one.");
  assert.ok(rows[1].result.indexOf('Failure') === 0,
    "resolving the newest pending entry must not disturb the earlier one: " +
        rows[1].result);
  ...
  assert.strictEqual(rows.length, 5, "expected exactly 5 entries, got " +
                     rows.length);
  assert.ok(times[0] >= times[times.length - 1],
            "the history should be newest-first.");
  ...
  assert.strictEqual(respRows.length, rows.length,
    "both pages should show the same log: " + respRows.length + " vs " +
        rows.length);
  ...
  var respOrder = await driver.executeScript(
    "var t = document.getElementById('pane_tools'), h = " +
        "document.getElementById('pane_history');" +
    "return !!(t.compareDocumentPosition(h) & " +
        "Node.DOCUMENT_POSITION_FOLLOWING);");
  assert.ok(respOrder, "the Operations History pane must come after the " +
            "Tools pane here as well.");
  ...
  await driver.navigate().refresh();
  await driver.wait(until.elementLocated(By.id('pane_history')), waitTime);
  respRows = await waitForRows(driver, 5,
      "the log vanished on the response page.");
  assert.strictEqual(respRows.map(function (r) { return r.result; }).join('|'),
                     beforeReload,
  ...
  await click(driver, By.xpath("//input[contains(@onclick, " +
              "\"saml_response.clearOperationHistory(\")]"));
  await driver.wait(async function () {
  ...
  await driver.wait(until.elementLocated(By.id('pane_history')), waitTime);
  assert.strictEqual((await historyRows(driver)).length, 0,
                     "the cleared history came back after a reload.");
