...
var log = bunyan.createLogger({ name: "ldp_vc_refresh",
    level: appconfig.logLevel || "info" });

if (typeof globalThis.btoa !== "function") {
  globalThis.btoa = function (s) {
    log.debug("Entering btoa().");
    log.debug("Leaving btoa().");
    return Buffer.from(s, "binary").toString("base64");
  };
  globalThis.atob = function (s) {
    log.debug("Entering atob().");
    log.debug("Leaving atob().");
...
const sdJwtVc = paths.requireSharedModule(
  [path.join(__dirname, "sd_jwt_vc.js"), path.join(ROOT, "client", "src",
   "sd_jwt_vc.js")],
...
async function requestCredential(meta, accessToken, holderKey,
                                 configurationId) {
...
// The credential as the wallet would store it, via the same two functions step
// 4 calls (vc_issuance_4.js reads extractCredential at the reissue and deferred
// paths and allCredentials when it keeps the result).
function walletReads(response) {
  log.debug("Entering walletReads().");
  const stored = wallet.extractCredential(response.body);
  log.debug("Leaving walletReads().");
...
// property of this format. BBS Sign is DETERMINISTIC — unlike ProofGen, it
// draws no randomness — and an ldp_vc carries no salts. So the credential is a
// pure function of (claims, holder key, timestamps), and the only part of that
// which varies between two requests is validFrom/validUntil/proof.created,
// stamped to the second. Two refreshes inside one second are therefore
// BYTE-IDENTICAL.
//
// dc+sd-jwt hides this: a fresh random salt per Disclosure makes every issuance
// differ whatever the clock says. That is why this wait belongs in this file
// and not in the SD-JWT one.
...
async function test() {
  ...
  assert.strictEqual(initial.status, 200,
                     "the mock should issue a token set: " + initial.raw);
  const firstRefreshToken = (initial.body || {}).refresh_token;
  assert.ok(firstRefreshToken,
    "a refresh needs a refresh token to start from, and section 14.5's first " +
        "call is exactly the " +
    "RFC 6749 grant that consumes one. Without it only the 14.3 route below " +
        "is possible.");
  ...
  const first = await requestCredential(meta, initial.body.access_token,
      boundKey);
  assert.ok(first.ok, "the first issuance should succeed: HTTP " +
            first.statusCode + " " +
    String(first.raw).slice(0, 200));
  const held = walletReads(first);
  assert.ok(held.stored,
            "the wallet should be able to store the first credential.");
  ...
  assert.ok(/^did:jwk:/.test(heldSubjectId),
    "for ldp_vc the holder is named by credentialSubject.id as a " +
        "did:jwk. Got: " + heldSubjectId);
  ...
  assert.ok(bound, "step 4 must resolve the holder key for an ldp_vc, not " +
            "report it as unbound. It is " +
    "in credentialSubject.id as a did:jwk; there is no cnf claim to read.");
  assert.strictEqual(bound.kty, boundKey.publicJwk.kty,
                     "and resolve it to a key of the right type.");
  assert.strictEqual(bound.crv, boundKey.publicJwk.crv, "on the right curve.");
  assert.strictEqual(bound.x, boundKey.publicJwk.x,
    "and it must be THE key the wallet proved possession of — the " +
        "coordinates step 4 compares against " +
  ...
  assert.strictEqual(sdJwtVc.bindingMemberName(held.parsed),
                     "credentialSubject.id",
    "and the pane must NAME it correctly: saying cnf.jwk on a credential " +
        "that has no cnf claim is how " +
  ...
  assert.ok(sdJwtVc.credentialLabel(held.parsed),
    "and the pane must show that type rather than an em dash — reading " +
        "payload.vct gives nothing here.");
  ...
  assert.strictEqual(window_.notBeforeMember, "validFrom",
                     "an ldp_vc's window is validFrom/validUntil.");
  assert.strictEqual(window_.expiresMember, "validUntil",
                     "not nbf/exp, which it does not carry.");
  assert.ok(window_.expires,
    "and the pane must find an expiry. Reading payload.exp gives null and " +
        "the pane then says \"no exp " +
    "claim, so it does not expire on its own\" about a credential that " +
        "expires in a month — wrong, and " +
    "wrong in the direction that stops someone refreshing it.");
  assert.strictEqual(window_.expires,
                     Math.floor(Date.parse(heldCredential.validUntil) / 1000),
    "resolved from the credential's own validUntil, as seconds since the " +
        "epoch so the pane formats " +
    "every format's dates the same way.");
  assert.ok(window_.expires > window_.notBefore,
            "and a window that runs forwards.");
  log.info("[pane] OK — type \"" + sdJwtVc.credentialLabel(held.parsed) +
           "\" (" +
    sdJwtVc.typeMemberName(held.parsed) + "), expiring " +
                           heldCredential.validUntil + " (" +
  ...
  assert.strictEqual(refreshed.status, 200,
    "the refresh_token grant should succeed: " + String(refreshed.raw).slice(0,
        200));
  ...
  const before = JSON.parse(Buffer.from(initial.body.access_token.split(".")[1],
      "base64url"));
  const after = JSON.parse(Buffer.from(newAccessToken.split(".")[1],
      "base64url"));
  assert.strictEqual(after.sub, before.sub,
    "the refreshed token must describe the SAME End-User — a refresh " +
        "re-authorizes, it does not " +
    "re-authenticate somebody else.");
  assert.ok(after.iat >= before.iat,
            "and be no older than the one it replaces.");
  ...
  assert.strictEqual(bogus.status, 400,
    "an invalid refresh token must be refused — otherwise the success above " +
        "says nothing. Got HTTP " +
    bogus.status);
  assert.strictEqual((bogus.body || {}).error, "invalid_grant",
                     "and refused as invalid_grant.");
  log.info("[14.5/1] OK — new access token for the same subject; a bogus one " +
           "is refused.");

  // --- 14.5, call 2: the Credential Request, reusing the bound key ----------
  log.info("=== Section 14.5, call 2: the same Credential Request, " +
           "same key ===");
  ...
  assert.ok(reissue.ok, "the credential endpoint should accept the REFRESHED " +
            "access token: HTTP " +
  ...
  assert.strictEqual(typeof got.stored, "string",
    "step 4 stores the refreshed credential the same way step 2 does — as a " +
        "string, because " +
    "localStorage takes one.");
  assert.ok(got.stored,
    "extractCredential() returned nothing for a refreshed ldp_vc. Step 4 " +
        "would show \"the response " +
    "carries no credential\" for a credential the issuer did send. Response: " +
    JSON.stringify(reissue.body).slice(0, 240));
  assert.strictEqual(got.all.length, 1,
                     "one proof was sent, so one credential comes back.");
  assert.strictEqual(got.all[0], got.stored,
    "and the two readers must agree: step 4 writes CREDENTIAL from one and " +
        "CREDENTIALS from the other.");
  ...
  assert.ok(Date.parse(refreshedCredential.validFrom) > Date.parse(
            heldCredential.validFrom),
    "the refreshed credential should be valid FROM later than the one it " +
        "replaces (" +
    refreshedCredential.validFrom + " vs " + heldCredential.validFrom +
        "). A refresh that does not " +
    "move the validity window has given the holder nothing.");
  assert.ok(Date.parse(refreshedCredential.validUntil) > Date.parse(
            heldCredential.validUntil),
    "and expire later — that is the point of refreshing it.");
  assert.notStrictEqual(refreshedCredential.proof.proofValue,
                        heldCredential.proof.proofValue,
    "and so carry a different proof. Note this follows from the timestamps " +
        "and NOT from any " +
    "randomness: BBS Sign is deterministic and an ldp_vc has no salts, so " +
        "the same claims signed by " +
    "the same key at the same second give identical bytes. That is checked " +
        "directly below.");
  assert.strictEqual(subjectIdOf(refreshedCredential), heldSubjectId,
    "reusing the holder key must give the SAME credentialSubject.id: that is " +
        "what makes this a " +
    "replacement for the credential in hand rather than a second one beside " +
        "it. For dc+sd-jwt the " +
    "same fact is cnf.jwk; this format has no cnf, so the subject id is the " +
        "only signal.");
  log.info("[14.5/2] OK — a new proof, same subject id: this replaces the " +
           "credential in hand.");
  ...
  assert.deepStrictEqual(claimsAfter, claimsBefore,
    "this issuer refreshes the SIGNATURE and not the claim values, so the " +
        "comparison step 4 shows " +
    "should report no changed claims. If the mock is later made to vary a " +
        "claim, this assertion is " +
    "the one to update — deliberately, rather than discovering it as a " +
        "surprise in the pane.");
  ...
  const km =
      (await common.httpJson(refreshedCredential.proof.verificationMethod))
      .body;
  ...
  assert.strictEqual(verdict.ok, true,
    "the refreshed credential's base proof must verify against the issuer's " +
        "published BBS key.");
  ...
  assert.strictEqual((await stsSuite.verifyBase(tampered, pk)).ok, false,
    "and an edited claim must break it — the control that shows the check " +
        "above is not always-true.");
  log.info("[14.5/2] OK — the refreshed proof verifies over " +
  ...
  if (sameSecond) {
    ...
    log.info("[determinism] straddled a second boundary — the identity check " +
  ...
  assert.ok(rebound.ok,
            "the issuer should bind a refreshed credential to a new key too.");
  ...
  assert.notStrictEqual(subjectIdOf(reboundCredential), heldSubjectId,
    "a new holder key must give a DIFFERENT credentialSubject.id — otherwise " +
        "the wallet cannot tell " +
    "the two credentials apart, which is the confusion section 14.5 " +
        "warns about.");
  assert.strictEqual(subjectIdOf(heldCredential), heldSubjectId,
    "and the credential in hand is untouched by that: it still needs its own " +
        "key, which is why step 4 " +
  ...
  const reboundParsed =
      sdJwtVc.parseCredential(wallet.extractCredential(rebound.body));
  ...
  assert.notStrictEqual(JSON.stringify(reboundBound), JSON.stringify(heldBound),
    "and must compare as CHANGED against the credential in hand — this is " +
        "what makes step 4 warn that " +
    "the refresh does not replace the old credential. Comparing an " +
        "unresolvable binding gives null vs " +
    "null, which reads as 'same key' and suppresses the warning exactly when " +
        "it matters.");
  const sameKeyParsed = sdJwtVc.parseCredential(got.stored);
  assert.strictEqual(JSON.stringify(sdJwtVc.boundHolderJwk(sameKeyParsed)),
                     JSON.stringify(heldBound),
    "while the same-key refresh must compare as UNCHANGED — the control, " +
        "without which the assertion " +
    "above would also pass for a comparison that always reports a change.");
  log.info("[key mode] OK — a new key gives a distinct subject id and " +
           "compares as changed; the " +
  ...
  if ((meta.credential_configurations_supported || {})[sdJwtConfig]) {
    ...
    if (sdResponse.ok) {
      const sdParsed =
          sdJwtVc.parseCredential(wallet.extractCredential(sdResponse.body));
      assert.strictEqual(sdJwtVc.bindingMemberName(sdParsed), "cnf.jwk",
        "a dc+sd-jwt credential is still bound by cnf.jwk — the shared " +
            "helper must not have renamed " +
      ...
      assert.strictEqual(sdBound.x, boundKey.publicJwk.x,
        "to the same holder key, read out of cnf.jwk rather than " +
            "credentialSubject.id.");
      assert.strictEqual(sdJwtVc.typeMemberName(sdParsed), "vct",
        "and an SD-JWT VC is still identified by its vct.");
      assert.strictEqual(sdJwtVc.credentialLabel(sdParsed), (sdParsed.payload ||
                         {}).vct,
        "shown from the vct claim, not from a type array it does not have.");
      const sdWindow = sdJwtVc.validityWindowOf(sdParsed);
      assert.strictEqual(sdWindow.expiresMember, "exp",
                         "and its window is still nbf/exp.");
      assert.strictEqual(sdWindow.expires, (sdParsed.payload || {}).exp,
        "read straight off the NumericDate claim, with no date parsing " +
            "in the way.");
      log.info("[cross-format] OK — dc+sd-jwt still reads vct, nbf/exp " +
               "and cnf.jwk.");
    } else {
      log.info("[cross-format] the issuer refused a " + sdJwtConfig +
               " request (HTTP " +
        sdResponse.statusCode +
            "); the dc+sd-jwt side of the helper was not exercised.");
    }
  } else {
    log.info("[cross-format] this issuer offers no \"" + sdJwtConfig +
             "\" configuration; the " +
  ...
  // jwkFromDid moved into sd_jwt_vc.js when step 4 started needing it, so it
  // now has TWO callers with different purposes: this format's HOLDER key, and
  // step 3's resolution of an ISSUER key (walt.id signs with a did:jwk). The
  // cases below are the ones this suite's own issuer cannot exercise — its
  // identifiers carry no fragment — and dropping the fragment strip is a silent
  // corruption: the base64url decode of "<jwk>#0" either throws or yields
  // nonsense, so an issuer key that used to resolve stops resolving and the
  // credential is reported unverifiable.
  const sampleJwk = { crv: "P-256", kty: "EC", x: boundKey.publicJwk.x,
      y: boundKey.publicJwk.y };
  ...
  assert.deepStrictEqual(sdJwtVc.jwkFromDid("did:jwk:" + encoded + "#0"),
                         sampleJwk,
    "and a did:jwk URL with a fragment must decode to the same key — " +
        "walt.id's issuer identifies its " +
    "signing key as did:jwk:<jwk>#0, so a decoder that keeps the fragment " +
        "resolves nothing.");
  assert.strictEqual(sdJwtVc.jwkFromDid("did:key:z6Mk"), null,
                     "another DID method is not this one.");
  assert.strictEqual(sdJwtVc.jwkFromDid("did:jwk:!!not-base64url"), null,
    "and an undecodable body is null rather than a throw — callers have " +
        "other resolution paths.");
  assert.strictEqual(sdJwtVc.jwkFromDid(""), null, "as is nothing at all.");
  log.info("[did:jwk] OK — bare and fragment forms decode alike; junk is " +
           "null, not a throw.");
  ...
  const legacy = sdJwtVc.validityWindowOf({
    format: "ldp_vc",
    payload: { issuanceDate: "2026-01-01T00:00:00Z",
              expirationDate: "2027-01-01T00:00:00Z" }
  });
  assert.strictEqual(legacy.notBeforeMember, "issuanceDate",
    "a VCDM 1.1 credential names its window differently, and the pane should " +
        "say which it read.");
  assert.strictEqual(legacy.expiresMember, "expirationDate", "both members.");
  assert.strictEqual(legacy.expires,
                     Math.floor(Date.parse("2027-01-01T00:00:00Z") / 1000),
  ...
  const both = sdJwtVc.validityWindowOf({
    format: "ldp_vc",
    payload: { validUntil: "2027-06-01T00:00:00Z",
              expirationDate: "2020-01-01T00:00:00Z" }
  });
  assert.strictEqual(both.expiresMember, "validUntil",
                     "VCDM 2.0's spelling takes precedence.");
  assert.strictEqual(both.expires,
                     Math.floor(Date.parse("2027-06-01T00:00:00Z") / 1000),
    "and its value is the one used — picking the 1.1 member here would " +
        "report this credential as " +
  ...
  const bad = sdJwtVc.validityWindowOf({ format: "ldp_vc",
      payload: { validUntil: "soon" } });
  assert.strictEqual(bad.expires, null,
    "a malformed date is absent, not the epoch. Date.parse returns NaN and " +
        "Math.floor(NaN/1000) is " +
    "NaN, but a truthiness test on it would fall through to \"EXPIRED\" " +
        "arithmetic.");
  log.info("[validity] OK — 1.1 and 2.0 spellings, 2.0 wins, junk reads " +
           "as absent.");
  ...
  const again = await requestCredential(meta, initial.body.access_token,
      boundKey);
  assert.ok(again.ok,
    "the Credential Endpoint may be asked again with an access token that is " +
        "still valid (section " +
    "14.3). This is the only refresh route after the pre-authorized code " +
        "grant, which issues no " +
    "refresh token. HTTP " + again.statusCode + " " + String(again.raw).slice(0,
        200));
  const againRead = walletReads(again);
  assert.ok(againRead.stored,
            "and the wallet reads that response the same way.");
  ...
  log.info("[14.3] OK — a still-valid access token gets a fresh credential " +
           "with no token call.");
  ...
  assert.strictEqual(wallet.extractCredential({ transaction_id: "t-1" }), "",
    "a deferred response carries no credential, and step 4 routes on that " +
        "emptiness.");
  assert.strictEqual(wallet.extractCredential(null), "",
                     "nor does a missing body.");
  log.info("[reader] OK — and still reports nothing for a deferred/empty " +
           "response.");
