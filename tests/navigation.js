...
var cardByTitle = function (title) {
  ...
  return By.xpath("//a[contains(@class,'landing-card')]" +
                  "[.//span[contains(@class," +
                      "'landing-card-title')][normalize-space()=\"" + title +
                      "\"]]");
};
...
async function checkFooterVersion(driver, where) {
  ...
  assert.ok(/Build \S+ — built /.test(title || ''),
    "[" + where +
        "] the version tooltip should carry the build provenance, found: '" +
        title + "'");
  if (seenVersion === null) { seenVersion = text; log.info("Footer version: " +
      text); }
  else {
    assert.strictEqual(text, seenVersion,
      "[" + where + "] every page must report the same build: " + text +
          " vs " + seenVersion);
  }
...
async function checkStylesheetsLoaded(driver, where) {
  ...
  var m = await driver.executeScript(
    ...
    "var empty = [];" +
    "var defined = {};" +
    "Array.prototype.slice.call(document.styleSheets).forEach(function " +
        "(sheet) {" +
    "  var rules = null;" +
    "  try { rules = sheet.cssRules; } catch (e) { rules = null; }" +
    "  if (!rules || !rules.length) { if (sheet.href) " +
        "empty.push(sheet.href); return; }" +
    ...
    "  Array.prototype.slice.call(e.classList).forEach(function (c) { " +
        "used[c] = true; }); });" +
    "return { links: links, empty: empty, sheets: " +
        "document.styleSheets.length," +
    "         used: Object.keys(used), defined: Object.keys(defined) };");

  assert.ok(m.sheets > 0, "[" + where +
            "] the page loaded no stylesheets at all.");
  assert.deepStrictEqual(m.empty, [],
    "[" + where + "] these stylesheets loaded with no rules in them (a 404 " +
        "serving an error page, or an " +
    "empty file): " + m.empty.join(", "));

  var definedSet = {};
  m.defined.forEach(function (c) { definedSet[c] = true; });
  var unstyled = m.used.filter(function (c) { return STYLED_PREFIXES.test(c) &&
      !definedSet[c]; });
  assert.deepStrictEqual(unstyled, [],
    "[" + where + "] these styling classes are used but defined in none of " +
        "the page's stylesheets: " +
    unstyled.join(", ") +
        ". The page is probably linking the wrong sheet — it links: " +
        m.links.join(", "));
  log.info("[styles] " + where + " — " + m.sheets +
           " stylesheet(s), all with rules, and every " +
...
// wrong, twice: six cards in the two-wide grid the page had when there were
// four of them made three rows, and a seventh in the three-wide grid that
// replaced it would have done the same. A choice you have to scroll to find is
// one you do not know you have, so it is checked geometrically rather than by
// eye.
//
// Also checked here, because both were real and neither is visible in a
// screenshot: a card nested INSIDE another (an unclosed <a> in the markup,
// which the browser silently recovers from) and a card whose icon has no accent
// colour of its own (landing.css carries one rule per card, so a new protocol
// needs one or it inherits the text colour).
// ---------------------------------------------------------------------------
var DEFAULT_TEXT_COLOUR = "rgb(51, 51, 51)";
// How much of a 768px-tall screen the cards may occupy. The viewport check
// below is not enough on its own: headless Chrome has no tab strip, toolbar or
// bookmark bar, so window.innerHeight there is the whole 768 while a real
// browser leaves roughly 620-640. Measured 2026-07-30 at 571px with seven cards
// in a 4 + 3 grid, so this budget is the slack that remains — an eighth
// protocol has to fit inside it, which most likely means another column or
...
async function landingFitsOnOneScreen(driver) {
  ...
  var m = await driver.executeScript(
    ...
    "  var row = rows.filter(function (r) { return Math.abs(r.top - top) < " +
        "12; })[0];" +
    ...
    "  horizontalScroll: document.documentElement.scrollWidth > " +
        "window.innerWidth + 1," +
    "  nested: document.querySelectorAll('a.landing-card " +
        "a.landing-card').length," +
    "  titles: cards.map(function (c) {" +
    "    var t = c.querySelector('.landing-card-title'); return t ? " +
        "t.textContent.trim() : ''; })," +
    "  iconColours: cards.map(function (c) {" +
    "    var i = c.querySelector('.landing-icon'); return i ? " +
        "getComputedStyle(i).color : ''; })" +
    "};");

  try {
    assert.ok(m.count >= 8,
              "the landing page should offer every protocol; found " + m.count +
              " card(s).");
    assert.strictEqual(m.nested, 0,
      "no protocol card may be nested inside another — that means an " +
          "unclosed <a> in index.html. Found " +
      m.nested + ".");
    assert.ok(m.lowestCardBottom <= m.viewportHeight,
      "every protocol card must be visible without scrolling at 1366x768: " +
          "the lowest card ends at " +
      m.lowestCardBottom + "px, the viewport is " + m.viewportHeight +
          "px. Rows: [" + m.rows.join(", ") +
      "]. Adding a protocol means making the cards fit, not letting them run " +
          "off the screen.");
    assert.ok(!m.horizontalScroll, "and the page must not scroll sideways.");
    assert.ok(m.lowestCardBottom <= CARD_HEIGHT_BUDGET,
      "the cards must also fit the " + CARD_HEIGHT_BUDGET +
          "px a REAL browser leaves on a 768px screen " +
      "once its tab strip and toolbar are accounted for (headless has " +
          "neither, so the viewport check above " +
      "passes too easily). The lowest card ends at " + m.lowestCardBottom +
          "px.");
    ...
    assert.ok(m.rows.length >= 2,
              "the cards should be laid out in rows, not one long column.");
    assert.ok(m.rows[0] >= 4,
      "the first row should hold at least four cards at this width; got " +
          m.rows[0] + ".");
    m.iconColours.forEach(function (colour, i) {
      assert.notStrictEqual(colour, DEFAULT_TEXT_COLOUR,
        "the '" + m.titles[i] + "' card's icon has no accent colour of its " +
            "own — landing.css needs a " +
        ":nth-child(" + (i + 1) + ") rule. Got " + colour + ".");
    });
    log.info("[landing] OK — " + m.count + " cards in rows of [" +
             m.rows.join(", ") + "], all visible " +
             "(lowest ends " + m.lowestCardBottom + "px of " +
                 m.viewportHeight + "px), each with its own " +
             "accent colour.");
  } finally {
...
async function navigationActivities(driver) {
  ...
  // needs the WS-Federation Keycloak side-car and is skipped without it, so
  // this is the only place the page is loaded on every run — which is how it
  // came to be served unstyled for a while without anything failing.
