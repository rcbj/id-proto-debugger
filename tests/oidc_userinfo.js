...
//
// For each: click its UserInfo Data link, press Retrieve UserInfo on the page
// it lands on WITHOUT touching the configuration, check the response describes
// the user who signed in, and come back with "Return to debugger".
//
...
// only way the refresh link carrying the stale pre-refresh token, or the
// history link carrying the live token instead of the generation it names,
// would show up.
//
// "Use the default configuration" is asserted rather than assumed, and on a
// build that has the api the default is the back-end — so this job needs the
// api service, as a user pressing that button with an untouched page does. The
// markup reads the other way and is worth knowing about: the front-end radio
// carries checked="true" and the back-end one checked="false", but
// `checked="false"` is still the attribute being PRESENT, so both are checked
// in the source and the last of two radios sharing a name wins. The effective
// default therefore matches every other initiation pane in the debugger; the
// front-end checked="true" is dead intent.
//
...
// call goes browser -> OP directly. The default assertion reads which of the
// two builds it is off the page rather than assuming, and holds either way; see
...
// clickStable() and valueOf() — the stale-tolerant click and read this test
// used to carry its own copies of — now live in common/tests.js, required
// above. They moved there when tests/oauth2_token_revocation.js hit the same
// window on its second revocation; the reasoning is in that file.

// ---------------------------------------------------------------------------
// Tokens, through the Authorization Code flow.
// ---------------------------------------------------------------------------
async function obtainTokens(driver, { clientId, scope, user }) {
  log.debug("Entering obtainTokens().");
  ...
  // that one is left at its own default, which is the point of the test.)
  ...
  log.debug("Leaving obtainTokens().");
  return accessToken;
}

// The refresh call, which is what fills the second pane. Its own results pane
// is separate from the first, and the access token in it is a NEW one — which
// is the whole reason the second UserInfo link is worth testing.
async function refreshTokens(driver) {
  ...
  }, waitTime * 6, "The refresh call produced no new access token in the " +
      "refresh results pane.");
  log.info("Leaving refreshTokens().");
  log.debug("Leaving refreshTokens().");
  return refreshed;
}
...
async function exerciseUserInfoLink(driver, source, expected) {
  log.debug("Entering exerciseUserInfoLink().");
  ...
  assert.ok(pane.length, "The \"" + source.name + "\" pane (#" + source.pane +
            ") is not on the page.");
  const links = await pane[0].findElements(By.css(source.link));
  assert.strictEqual(links.length, 1,
    "The \"" + source.name +
        "\" pane should hold exactly one UserInfo Data link (" + source.link +
  ...
  assert.ok(paneAccessToken && paneAccessToken.split(".").length === 3,
    "The \"" + source.name +
        "\" pane holds no access token to ask UserInfo about.");
  const paneIdToken = await valueOf(driver, source.idTokenField);
  assert.ok(paneIdToken && paneIdToken.split(".").length === 3,
    "The \"" + source.name +
        "\" pane holds no ID token, so it should not be offering a UserInfo " +
    "Data link at all.");

  await clickStable(driver, By.css(source.link),
                    "the UserInfo Data link in \"" + source.name + "\"",
                    { within: source.pane });
  ...
  // BEING PRESENT, so both are checked in the source and, for two radios
  // sharing a name, the last one wins. The front-end `checked="true"` is dead
  // intent.
  //
  ...
  // therefore selects the front-end radio and DISABLES the back-end one.
  // Pinning "the back-end is selected" unconditionally fails there for a reason
  // that has nothing to do with UserInfo — which is what it did against
  ...
  // Either way the assertion still catches a silent switch — a backed build
  // that stopped defaulting to the api, or a backend-less one that left the
  // disabled api option selected and so would initiate nothing.
  const backEnd =
      await driver.findElement(By.id("userinfo_initiateFromBackEnd"));
  const backEndAvailable = await backEnd.isEnabled();
  if (backEndAvailable) {
    assert.ok(await backEnd.isSelected(),
      "The UserInfo page no longer defaults to initiating the call from the " +
          "api. This test presses " +
      "Retrieve UserInfo without touching the configuration, so a changed " +
          "default silently exercises " +
      "a different path.");
  } else {
    const frontEnd =
        await driver.findElement(By.id("userinfo_initiateFromFrontEnd"));
    assert.ok(await frontEnd.isSelected(),
      "The UserInfo page has disabled api initiation (a build with " +
          "backendAvailable: false), but has " +
      "not selected the browser instead — so pressing Retrieve UserInfo " +
          "would initiate the call from " +
      "nowhere.");
    log.info("This target has no api (the back-end radio is disabled), so " +
             "the call is initiated from " +
             "the browser — which needs the OP to send CORS headers for " +
                 "this origin.");
  }
  const endpoint = await valueOf(driver, "userinfo_endpoint");
  assert.strictEqual(endpoint, expected.userinfoEndpoint,
    "The UserInfo page is pointed at \"" + endpoint +
        "\", not at the endpoint discovery published (" +
    expected.userinfoEndpoint + ").");

  // THE check this test exists for: the page carries the token from the pane
  // the link was in. All three sets describe the same user, so a wrong token
  // here produces a response that looks perfectly correct.
  const loaded = await valueOf(driver, "token_access_token");
  assert.strictEqual(loaded, paneAccessToken,
    "The UserInfo page loaded a different access token from the one in the \"" +
        source.name +
    "\" pane. The response would still look right — every token here belongs " +
        "to the same user — " +
    "so this is the only place the wrong one shows up.");

  await clickStable(driver,
      By.css("input[type=\"submit\"][value=\"Retrieve UserInfo\"]"),
                    "Retrieve UserInfo");
  ...
  try {
    ...
    throw new Error("The UserInfo call from \"" + source.name +
                    "\" produced no JSON. The output box " +
                    "holds: \"" + output.trim().slice(0, 300) + "\".");
  }
  const claims = JSON.parse(output);
  log.info("[" + source.name + "] UserInfo returned: " +
           Object.keys(claims).sort().join(", "));

  // Who it describes. preferred_username is the login name on both OPs; `sub`
  // is opaque and is compared against the ID token from the SAME pane, which
  // ties the response to the token that fetched it.
  assert.strictEqual(claims.preferred_username, expected.user,
    "UserInfo from \"" + source.name + "\" describes " +
  ...
  assert.strictEqual(claims.sub, claimsOf(paneIdToken).sub,
    "UserInfo answered about " + claims.sub +
        ", but the ID token in the same pane describes " +
    claimsOf(paneIdToken).sub +
             ". OIDC Core section 5.3.2 requires them to match.");
  log.info("[" + source.name + "] OK — UserInfo describes " +
           claims.preferred_username +
  ...
  log.debug("Leaving exerciseUserInfoLink().");
  return claims;
}
...
//
// Generation 1 on purpose: it is the set the ORIGINAL flow produced, and by
// this point the live panes hold the refreshed set. So the history link is
// asked for a token that is demonstrably not the current one, which is the only
// way to tell "the link carries the generation it names" from "the link carries
// whatever is latest".
//
...
async function activateHistoryGeneration(driver, generation) {
  log.debug("Entering activateHistoryGeneration().");
  ...
  await driver.wait(until.elementIsVisible(driver.findElement(panel)),
                    waitTime * 3);
  const button = By.xpath(
      "//div[@id='token-history-panel']//tr[td[1][normalize-space()='" +
                          generation + "']]//input[@value='Activate']");
  const found = await driver.findElements(button);
  assert.strictEqual(found.length, 1,
    "Token History has no Activate button for generation " + generation +
        " (found " + found.length +
    "). Either the refresh call did not add a second set, or that generation " +
        "is already active.");
  await clickStable(driver, button, "the Activate button for generation " +
                    generation);
  await driver.wait(async function () {
  ...
  }, waitTime * 4, "Activating generation " + generation +
      " did not fill the Currently Viewing pane.");
  log.info("Leaving activateHistoryGeneration().");
  log.debug("Leaving activateHistoryGeneration().");
}
...
async function test() {
  ...
  try {
    ...
    log.info("[refresh] OK — the refresh call produced a second, different " +
             "token set.");
    const second = await exerciseUserInfoLink(driver, SOURCES[1], expected);

    // The third set comes from Token History, which needs a set activated
    // first.
    ...
    assert.strictEqual(cvToken, originalAccessToken,
      "Currently Viewing shows generation 1, so it should hold the access " +
          "token the original flow " +
      "produced. It holds a different one — which would make the UserInfo " +
          "answer right by accident.");
    assert.notStrictEqual(cvToken, refreshedAccessToken,
      "Currently Viewing is showing the REFRESHED token while claiming to " +
          "show generation 1.");
    log.info("[history] OK — generation 1 activated, holding the original " +
             "flow's access token.");
    ...
    // assertions above would all pass if each pane somehow described a
    // different person, which would mean the links had been crossed with
    // another session.
    assert.strictEqual(first.sub, second.sub,
      "The refresh call's UserInfo answer describes a different subject from " +
          "the original.");
    assert.strictEqual(first.sub, third.sub,
      "Token History's UserInfo answer describes a different subject from " +
          "the original.");
    log.info("[all three] OK — every UserInfo Data link answered for " +
             first.sub + ".");
