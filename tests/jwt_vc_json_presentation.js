...
//
// ---------------------------------------------------------------------------
// OID4VP presentation of a jwt_vc_json credential, against the mock Verifier
// the STS service hosts.
//
...
//
// The four places a format-blind implementation still LOOKS like it works,
// which are therefore what this asserts:
//
//   * nothing to select — the page must say why, not render an empty table;
//   * holder binding is the Verifiable Presentation JWT's own signature, where
//     an SD-JWT uses a Key Binding JWT;
//   * the DCQL paths are rooted at credentialSubject, so the wallet has to map
//     them onto the claim names it holds. Miss that and every requested claim
//     is reported WITHHELD while all of them were in fact sent — alarming to
//     the holder and false;
//   * the presentation over-discloses by construction, so "nothing more than
//     was asked for" would be a lie.
//
// Plus the negatives, built here rather than through the pages, because a
// wallet that only tests itself proves nothing about the verifier: a VP JWT
// signed by the wrong key, and a replay of accepted bytes into a second
// session.
// ---------------------------------------------------------------------------
...
async function presentThroughThePages(driver, held) {
  ...
  assert.deepStrictEqual(dcql.credentials[0].claims[0].path.slice(0, 1),
                         ["credentialSubject"],
    "and address its claims under credentialSubject, where a W3C VC keeps " +
        "them. Got: " +
  ...
  assert.strictEqual(presentation.split(".").length, 3,
                     "and is a three-part JWS.");

  const vpPayload = common.jsonFromB64u(presentation.split(".")[1]);
  assert.ok(vpPayload.vp &&
            [].concat(vpPayload.vp.verifiableCredential)[0] === held.credential,
  ...
  assert.strictEqual(vpPayload.nonce, request.params.nonce,
    "the VP JWT carries the request nonce — this format has no KB-JWT to " +
        "carry it.");
  ...
  assert.ok(!/were withheld/.test(sent),
    "nothing can be withheld in jwt_vc_json, so the summary must not say " +
        "anything was. Said: " + sent);
  assert.ok(/more than was asked for/.test(sent) && /jwt_vc_json/.test(sent),
    "and it should name the over-disclosure as a property of the " +
        "format. Said: " + sent);
  const answered = await text(driver, "vp_answered");
  assert.ok(!/NOT fully answered/.test(answered),
    "the request WAS answered — every claim it asked for went. Said: " +
        answered);

  log.info("[pages] OK — VP JWT accepted, nothing reported as withheld, " +
           "over-disclosure named.");
...
async function verifierNegatives(held, acceptedPresentation) {
  ...
  const signVp = function (privateKey, request, credential) {
    ...
    const payload = common.b64u(JSON.stringify({
      iss: "urn:holder", aud: request.params.client_id,
    ...
    const sig = common.b64u(crypto.sign(null, Buffer.from(header + "." +
        payload),
  ...
  const failedCheck = function (verdict) {
    log.debug("Entering failedCheck().");
    log.debug("Leaving failedCheck().");
  ...
  assert.strictEqual(v1.ok, false,
                     "a VP JWT signed by the wrong key must be refused.");
  assert.ok(/Holder binding/.test(failedCheck(v1)),
    "and refused for the holder binding specifically, not for something " +
        "incidental. Failed: " +
    failedCheck(v1));
  log.info("[verifier] OK — a VP JWT signed by the wrong key is refused (" +
           failedCheck(v1) + ").");
  ...
  assert.strictEqual(v2.ok, false,
    "replaying an accepted presentation into a new session must be refused — " +
        "otherwise the nonce buys " +
  ...
  log.info("[verifier] OK — a replayed presentation is refused (" +
           failedCheck(v2) + ").");

  // A tampered credential inside an otherwise valid presentation.
  const parts = held.credential.split(".");
  const tampered = parts[0] + "." + parts[1] + "." + parts[2].slice(0, -4) +
      "AAAA";
  ...
  log.info("[verifier] OK — a tampered credential is refused (" +
           failedCheck(v3) + ").");
  ...
  assert.strictEqual(v4.ok, false,
      "an SD-JWT answering a jwt_vc_json query must be refused.");
  assert.ok(/Format/.test(failedCheck(v4)),
    "and named as a format problem rather than an undecodable JWT. Failed: " +
        failedCheck(v4));
  log.info("[verifier] OK — an SD-JWT answering a jwt_vc_json query is " +
           "refused (" + failedCheck(v4) + ").");
  log.debug("Leaving verifierNegatives().");
}

async function test() {
  log.debug("Entering test().");
  log.info("Running jwt_vc_json presentation against issuer " + issuerBase +
  ...
  assert.ok(found.meta, "no credential issuer metadata at " + issuerBase +
            ". Start the STS mock.");
  assert.ok(found.id,
    "this issuer offers no jwt_vc_json credential configuration, so nothing " +
  ...
  try {
    ...
    assert.strictEqual(errors.length, 0, "the pages logged browser errors:\n" +
                       errors.join("\n"));
    // After the console check: these drive the verifier directly, not the
    // pages.
