...
//
// What is being tested is a chain, and every link of it is somewhere a
// credential could look fine and verify against nothing:
//
//   1. the issuer ADVERTISES the DID (credential issuer metadata, and the
//      jwt-vc-issuer document that SD-JWT VC's own key resolution uses), and
//      says per credential configuration which identifier its credentials will
//      carry. Without this a wallet learns the DID only from the credential
//      itself, which is the one source that cannot corroborate it;
...
//      Linkage Credential (DIF Well Known DID Configuration). For did:web this
//      is the link that is not circular: resolving did:web:host means fetching
//      host, so the DID document alone cannot establish that host's controller
//      and the DID's controller are one party;
//   4. the credentials issued from the DID configurations actually name the DID
//      — dc+sd-jwt in iss, ldp_vc in issuer and in its proof's
//      verificationMethod;
//   5. and the key resolved from the DID document VERIFIES those credentials. A
//      DID that resolves to the wrong key is the failure that looks like
//      success right up to the last step.
//
...
//     DID route is an extension — draft-ietf-oauth-sd-jwt-vc defines no
//     DID-based issuer signature mechanism — so the specification's own route
//     must go on being the one the plain configuration exercises. A test suite
//     where every credential had switched to DIDs would have stopped testing
//     the spec;
...
const metadataClient = paths.requireSharedModule(
  [path.join(__dirname, "metadata_client.js"), path.join(ROOT, "client", "src",
   "metadata_client.js")],
...
// The two configurations that name the issuer by DID, and the plain ones they
// are clones of. The pairing is the point: each DID case is checked against its
// sibling, so "the DID variant works" cannot be true by accident of it being
// different in some other way.
const PAIRS = [
  { did: "IdentityCredentialDid", plain: "IdentityCredential",
   format: "dc+sd-jwt" },
  { did: "IdentityCredentialLdpVcDid", plain: "IdentityCredentialLdpVc",
   format: "ldp_vc" }
];
...
function issuerOrigin() {
  log.debug("Entering issuerOrigin().");
  log.debug("Leaving issuerOrigin().");
...
async function metadataAdvertisesTheDid() {
  ...
  assert.ok(meta, "no credential issuer metadata at " + issuerBase +
            ". Start the STS mock.");

  assert.ok(meta.issuer_did,
    "the credential issuer metadata should name this issuer's DID. Without " +
        "it a wallet that receives " +
    "a credential whose iss is a did:web has been told about that DID by " +
        "nothing it fetched — it has " +
    "to take the credential's word for who issued it, which is the one thing " +
        "a credential cannot " +
    "establish about itself.");
  assert.ok(did.isDid(meta.issuer_did), 'issuer_did should be a DID; got "' +
            meta.issuer_did + '".');
  assert.strictEqual(did.parse(meta.issuer_did).method, "web",
    "this issuer's DID should be did:web — the only method with a document " +
        "to serve. did:jwk and " +
    "did:key resolve from the identifier itself, which would make the " +
        "endpoint pointless.");
  ...
  for (const pair of PAIRS) {
    ...
    assert.ok(didEntry, 'this issuer offers no "' + pair.did +
              '" configuration. Offered: ' +
      Object.keys(configs).join(", "));
    assert.ok(plainEntry, 'this issuer offers no "' + pair.plain +
              '" configuration to compare it with.');
    assert.strictEqual(didEntry.format, pair.format,
      pair.did + " should be " + pair.format +
          ", the same format as its sibling.");
    ...
    assert.strictEqual(didEntry.issuer_identifier, meta.issuer_did,
      pair.did +
          " should advertise that its credentials name the issuer by DID.");
    assert.strictEqual(plainEntry.issuer_identifier, meta.credential_issuer,
      pair.plain + " should advertise the https identifier it has always " +
          "used — if this has become " +
      "a DID, the whole issuer has been switched over and the " +
          "specification's own key-resolution " +
    ...
    assert.deepStrictEqual(strip(didEntry), strip(plainEntry),
      pair.did + " should differ from " + pair.plain +
          " in NOTHING but its scope, its display name " +
      "and the issuer identifier it advertises. Anything else and the two " +
          "are not comparable.");
    assert.notStrictEqual(didEntry.scope, plainEntry.scope,
      "the two configurations need distinct scopes, or a wallet asking by " +
          "scope cannot choose.");
  }
  log.info("[metadata] OK — both DID configurations advertise the DID and " +
           "are clones of their siblings.");

  // SD-JWT VC's own key-resolution document. Its `issuer` must stay the https
  // identifier — the specification has a verifier insert the well-known path
  // into the credential's iss and require that this document's issuer equals
  // what it started from, and a DID cannot be the subject of that rule. The DID
  // is named BESIDE it, which is what connects the two identifiers at all.
  const jwtVcIssuer = await common.httpJson(issuerBase +
      "/.well-known/jwt-vc-issuer");
  assert.ok(jwtVcIssuer.ok,
            "the jwt-vc-issuer document should be served: HTTP " +
            jwtVcIssuer.status);
  assert.strictEqual(jwtVcIssuer.body.issuer, meta.credential_issuer,
    "the jwt-vc-issuer document's issuer must remain the https identifier " +
        "this document is FOUND by.");
  assert.ok(jwtVcIssuer.body.jwks_uri,
            "and it should still publish a jwks_uri.");
  assert.strictEqual(jwtVcIssuer.body.issuer_did, meta.issuer_did,
    "and it should name the same DID the credential issuer metadata does. " +
        "Two documents disagreeing " +
    "about which DID this issuer answers to is worse than neither of " +
        "them saying.");
  log.info("[metadata] OK — jwt-vc-issuer keeps its https issuer and names " +
           "the DID beside it.");
...
async function theDidResolvesAndTheOriginProvesIt(meta) {
  ...
  assert.ok(methods.length >= 2,
    "this issuer signs two quite different things and should publish a key " +
        "for each: RS256 JWTs and " +
    "bbs-2023 Data Integrity proofs. Got " + methods.length + " method(s).");

  const jose = methods.filter(function (m) { return m.jwk; });
  const multibase =
      methods.filter(function (m) { return m.publicKeyMultibase; });
  assert.ok(jose.length >= 1, "at least one key must be expressible as a " +
            "JWK, or no JWS can be verified.");
  assert.ok(multibase.length >= 1,
    "the BBS key must appear as a publicKeyMultibase. A BLS12-381 key has no " +
        "registered JOSE kty, so " +
    "forcing it into a publicKeyJwk would be inventing one.");
  const converted = did.assertionJwks(resolved.document);
  assert.ok(converted.jwks.keys.length >= 1,
            "the document should offer a usable assertion key.");
  assert.ok(converted.unusable.length >= 1,
    "and should report the BBS key as one it cannot express as a JWK, by " +
  ...
  log.info("[did:web] OK — " + methods.length + " verification methods, " +
           converted.jwks.keys.length + " usable as JWKs, " +
               converted.unusable.length + " not.");

  log.info("=== Well Known DID Configuration ===");
  const linkage = await did.verifyOriginLinkage(issuerOrigin(), meta.issuer_did,
      RESOLVE);
  assert.ok(linkage.matched.length >= 1,
    "the origin's DID Configuration should carry a Domain Linkage Credential " +
        "for the DID this issuer " +
    "advertises. Found entries for: " +
    linkage.results.map(function (r) { return r.did || "(none)"; }).join(", "));
  const failed =
      linkage.matched[0].checks.filter(function (c) { return !c.ok; });
  assert.deepStrictEqual(failed.map(function (c) { return c.name + ": " +
                         c.detail; }), [],
    "every check on the Domain Linkage Credential should pass.");
  assert.strictEqual(linkage.linked, true,
    "and the verdict should be that this origin and this DID are the " +
        "same entity.");
  assert.strictEqual(linkage.matched[0].origin, issuerOrigin(),
    "the credential should name the origin it was served from, or it belongs " +
        "to a different site.");
  ...
  // vouch for a DID it does not carry. Otherwise "linked" would be a property
  // of the file existing rather than of what it says.
  const impostor = await did.verifyOriginLinkage(issuerOrigin(),
      "did:web:somebody.else", RESOLVE);
  assert.strictEqual(impostor.linked, false,
    "an origin that links its OWN DID must not read as vouching for " +
        "anybody else's.");
...
async function sdJwtVcNamesTheDidAndVerifies(meta) {
  ...
  assert.strictEqual(payload.iss, meta.issuer_did,
    "the credential's iss should be the DID the metadata advertised. If " +
        "these differ, a wallet that " +
    "believed the metadata is resolving the wrong identifier.");
  assert.ok(payload.cnf && payload.cnf.jwk,
    "holder binding must still be cnf.jwk (RFC 7800). A DID here would be " +
        "nobody's convention — " +
    "the DID names the ISSUER, and that is the whole of what this " +
        "extension changes.");
  ...
  const converted = did.assertionJwks(resolved.document);
  const verdict = await metadataClient.verifyJwsWithJwks(issuerJwt,
      converted.jwks, "the credential");
  assert.strictEqual(verdict.valid, true,
    "the credential must verify against a key the DID document authorises to " +
        "ASSERT. A DID that " +
    "resolves to the wrong key looks like success until exactly here.");
  log.info("[dc+sd-jwt] OK — iss is " + payload.iss +
           " and the resolved key verifies it (kid " +
           verdict.kid + ").");
  ...
  const strangerVerdict = await metadataClient.verifyJwsWithJwks(issuerJwt,
      strangerJwks, "the credential");
  ...
  assert.strictEqual(plainPayload.iss, meta.credential_issuer,
    "the PLAIN configuration must still name the issuer by its https " +
        "identifier, so that " +
    "/.well-known/jwt-vc-issuer — the mechanism draft-ietf-oauth-sd-jwt-vc " +
        "actually defines — goes " +
    "on being exercised. The DID route is an extension and must not quietly " +
        "replace it.");
  assert.ok(!did.isDid(plainPayload.iss),
            "and that identifier must not be a DID.");

  // Same claims from both: they are the same credential from the same issuer,
  // and only the issuer's own name differs.
  const claimNames = function (credential) {
    log.debug("Entering claimNames().");
    log.debug("Leaving claimNames().");
    return String(credential).split("~").slice(1).filter(Boolean)
                  .map(function (disclosure) {
      return common.jsonFromB64u(disclosure)[1];
    }).sort();
  };
  assert.deepStrictEqual(claimNames(held.credential),
                         claimNames(plain.credential),
    "the DID configuration should assert the same claims as its sibling — it " +
        "is the same credential " +
    "with the issuer named differently, not a different credential.");
  log.info("[dc+sd-jwt] OK — the plain sibling keeps its https iss and " +
           "asserts the same claims.");
...
async function ldpVcNamesTheDidAndVerifies(meta) {
  ...
  assert.strictEqual(credential.issuer, meta.issuer_did,
    "the credential's issuer should be the DID. VC Data Model 2.0 is " +
        "DID-native, so unlike " +
    "dc+sd-jwt this is ordinary rather than an extension.");

  const vm = credential.proof.verificationMethod;
  assert.ok(did.isDid(vm),
    "the proof's verificationMethod should be a DID URL into this issuer's " +
        "document, not an https " +
    'URL. Got "' + vm + '".');
  ...
  assert.ok(did.parse(vm).fragment,
    "a DID URL naming a key needs a fragment, or it names the document " +
  ...
  assert.strictEqual(pk.length, 96,
    "a BLS12-381 G2 public key is 96 compressed bytes; got " + pk.length +
        ". A short key here means " +
    "the multibase prefix was stripped twice.");

  const verdict = await stsSuite.verifyBase(credential, pk);
  assert.strictEqual(verdict.ok, true,
    "the embedded bbs-2023 proof must verify against the key resolved FROM " +
        "THE DID DOCUMENT. This is " +
    "the whole chain for this format: the credential names a DID, the DID " +
        "resolves to a document, the " +
    "document names the key, and the key verifies the proof.");
  log.info("[ldp_vc] OK — issuer " + credential.issuer +
           ", verificationMethod " + vm +
           ", proof verifies over " + verdict.statements.length +
               " statements.");
  ...
  assert.ok(!did.isDid(plain.credential.proof.verificationMethod),
    "and must keep an https verificationMethod, which " +
        "tests/ldp_vc_issuance.js dereferences with a " +
    'plain GET. Got "' + plain.credential.proof.verificationMethod + '".');
  const plainResolved = await did.resolveVerificationMethod(
      plain.credential.proof.verificationMethod,
                                                            RESOLVE);
  assert.ok(plainResolved.method.publicKeyMultibase,
    "and that URL must still dereference to a key — the same helper must " +
        "handle both forms.");
  log.info("[ldp_vc] OK — the plain sibling keeps its https issuer and " +
           "dereferenceable key URL.");

  const claimsOf = function (c) {
    log.debug("Entering claimsOf().");
    log.debug("Leaving claimsOf().");
    return Object.keys(c.credentialSubject).sort().join(",");
  };
...
program
  .name("vc_did")
  .description("Verify the issuer named by DID: advertisement, resolution, " +
      "domain linkage, both formats.")
