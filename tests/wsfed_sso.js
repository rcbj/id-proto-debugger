...
// ?id=<stash>), or, on the STATIC S3 + CloudFront deployments, the Lambda@Edge
// in infra/edge/wsfed_landing.js (POST /wsfed -> sessionStorage -> ?posted=1).
// The second one exists because the passive profile has no redirect response
// binding: SAML's static deployments dodge this by asking the IdP to return the
// response over HTTP-Redirect to a static page (see responseProtocolBinding()
// in saml_request.js), and WS-Federation has no such option — the token comes
// back as a POST or not at all, so a static site needs code at the edge to
// catch it. The assertions below are written against what the user sees, so
// they hold for either landing; tests/edge_landing_contract.js separately
// guards the one thing that cannot be checked through the browser, namely that
// the Lambda and the page still agree on the hand-off key names.
//
...
// Assert the Operations History (on whichever page is currently loaded)
// resolved the given operation to Success — a green saml-ok Result cell. This
// is the user-visible confirmation that the call a button dispatched actually
// completed: for the sign-out it proves the Logout button's request went out
// AND the IdP reported it done, not merely that a button was clicked.
async function assertOperationSuccess(driver, operationLabel, timeout) {
  log.debug("Entering assertOperationSuccess().");
  await driver.wait(async function () {
    return await driver.executeScript(function (op) {
      ...
      for (var i = 0; i < rows.length; i++) {
        ...
        return result.className.indexOf('saml-ok') >= 0 &&
                                        /Success/.test(result.textContent);
      }
      return false;
    }, operationLabel);
  }, timeout, "the Operations History did not record '" + operationLabel +
      "' as Success.");
...
function combinationFromEnv() {
  log.debug("Entering combinationFromEnv().");
  var binding = (process.env.WSFED_SIG_BINDING || "redirect").toLowerCase();
  log.debug("Leaving combinationFromEnv().");
  return {
    ...
    includeWreq: (process.env.WSFED_INCLUDE_WREQ || "") === "true" ||
                  (binding === "enveloped")
  };
}

function comboLabel(combo) {
  log.debug("Entering comboLabel().");
  var sign = combo.sign ? (combo.binding + "+" + combo.sigAlg) : "unsigned";
  log.debug("Leaving comboLabel().");
  return "mode=" + combo.mode + " sign=" + sign + " initiate=" +
      combo.initiate +
...
async function applyCombination(driver, combo, timeout) {
  log.debug("Entering applyCombination().");
  ...
  if (combo.initiate === "front") { await setRadio(driver,
      "wsfed_initiateFromFrontEnd"); }
  else { if (await elementExists(driver,
        "wsfed_initiateFromBackEnd")) await setRadio(driver,
        "wsfed_initiateFromBackEnd"); }

  // Optional passthrough params (wctx echoed back; the rest the IdP mostly
  // ignores). whr is intentionally omitted: it is a home-realm hint that needs
  // a federated IdP alias configured at the side-car, and an unknown value
  // breaks login — that would be an IdP-config failure, not a workflow one.
  if (combo.optionalParams) {
    ...
    await setField(driver, "wsfed_auth_type",
  ...
  if (!combo.sign) {
    ...
    if (await elementExists(driver,
        "wsfed_sign_request")) await setChecked(driver, "wsfed_sign_request",
        false);
    ...
    log.debug("Leaving applyCombination().");
    return;
  }
  ...
  assert(await elementExists(driver, "wsfed_sign_request"),
    "WSFED_SIGN=on but the 'Digitally sign request' control is absent — " +
        "rebuild the client bundle " +
    "(the request-signing feature must be compiled into " +
        "public/js/wsfed_request.js).");
  ...
  await setField(driver, "wsfed_sig_alg", SIG_ALG_URIS[combo.sigAlg] ||
                 SIG_ALG_URIS["rsa-sha256"]);

  if (combo.binding === "enveloped") {
    // Selecting the enveloped binding auto-enables the inline wreq; give it
    // real XML to sign.
    await setChecked(driver, "wsfed_include_wreq", true);
    await setField(driver, "wsfed_wreq", sampleWreqXml());
  }
...
async function assertSignatureGenerated(driver, combo, timeout) {
  log.debug("Entering assertSignatureGenerated().");
  ...
  if (combo.binding === "redirect") {
    assert(/signed query string/i.test(status),
      "expected a redirect-binding query-string signature; status was: " +
          status);
...
async function wsfedActivities(driver, metadataUrl, realm, user, combo) {
  ...
  try {
    ...
    log.warn("Metadata load/parse did not complete cleanly (" + e.message +
             ") — continuing with the derived endpoint.");
  }
  ...
  var endpoint = await driver.findElement(By.id("wsfed_signin_endpoint"))
      .getAttribute("value");
  if (!endpoint) { endpoint =
      deriveEndpoint(metadataUrl); await setField(driver,
      "wsfed_signin_endpoint", endpoint); }
  ...
  // end at the last hop with nothing to show. Assert it here rather than
  // letting the wait below time out saying only "the response page never
  // loaded".
  var reply =
      await driver.findElement(By.id("wsfed_reply")).getAttribute("value");
  ...
  assert(!/\/wsfed_response\.html(\?|$)/.test(reply),
    ...
    "On a static deployment it should be the /wsfed landing answered by the " +
        "Lambda@Edge — set " +
    "wsfedEdgeLanding: true in the client env config for this target and " +
        "redeploy the site bundle " +
    "(see infra/edge/wsfed_landing.js).");

  // Send the sign-in request (wa=wsignin1.0). Button value is "Call IdP (Sign
  // In)".
  ...
  // one by putting it in sessionStorage and passing ?posted=1. Either way the
  // page reports the same thing, so the assertion does not need to know which.
  log.info("Wait for the WS-Federation response page.");
  await driver.wait(until.urlContains("wsfed_response.html"), loginWait,
    "the IdP's POST never reached a landing that could forward it to the " +
        "response page. " +
    "On a static deployment that is what a missing (or not-yet-applied) " +
        "Lambda@Edge looks like: " +
    "CloudFront answers the POST to /wsfed from S3 with 403/405 and the " +
        "browser stops there.");
  ...
  var respXml = await driver.findElement(By.id("wsfed_response_xml"))
      .getAttribute("value");
  log.info("wresult (first 1500 chars):\n" + (respXml || "").substring(0,
           1500));

  // Core assertions: the wresult is a WS-Trust RSTR and the extracted token is
  // a SAML assertion.
  assert(respXml.indexOf("RequestSecurityTokenResponse") >= 0,
    "wresult is not a WS-Trust RequestSecurityTokenResponse — see the logged " +
        "wresult above.");
  ...
  try {
    ...
    var sig = await driver.findElement(By.id("wsfed_sig_status"))
        .getAttribute("value");
    log.info("Signature validation: " + sig);
  } catch (e) {
    log.warn("Signature validation step skipped: " + e.message);
  }
...
// ---------------------------------------------------------------------------
// Sign-out (wa=wsignout1.0), driven from the same page, after the sign-in
// above.
//
// The passive profile's sign-out is a top-level navigation like the sign-in,
// and it returns NO token — so there is nothing to inspect, and the only things
// worth asserting are what actually happened:
//
//   1. the browser came back to wreply as a sign-out and not as a sign-in: the
//      /wsfed landing gets no wresult, so it redirects to
//      wsfed_response.html?signout=<the wa it received, or 1>, and the page
//      says so. Note that the wa on that final hop is the extension's choice,
...
async function wsfedSignOutActivities(driver, metadataUrl, realm) {
  ...
  var pre = await driver.executeScript(
    "return { signout: (document.getElementById('wsfed_signout_endpoint') || " +
        "{}).value || ''," +
    "         signin: (document.getElementById('wsfed_signin_endpoint') || " +
        "{}).value || ''," +
    "         realm: (document.getElementById('wsfed_realm') || " +
        "{}).value || ''," +
    "         reply: (document.getElementById('wsfed_reply') || " +
        "{}).value || '' };");
  if (!pre.signin) { await setField(driver, "wsfed_signin_endpoint",
      deriveEndpoint(metadataUrl)); }
  if (!pre.realm) { await setField(driver, "wsfed_realm", realm); }
  // wreply is where the IdP sends the browser afterwards, and it is what makes
  // the sign-out observable here at all. The page defaults it to the /wsfed
  // landing — Express's or the edge one, depending on the deployment; if that
  // default is missing there is no landing at all and this leg cannot be
  // checked, which is worth saying rather than guessing a URL.
  assert(pre.reply,
    "wsfed_reply (wreply) is empty, so the IdP has nowhere to return the " +
  ...
  assert(["1", "wsignout1.0", "wsignoutcleanup1.0"].indexOf(flag) >= 0,
    "the sign-out landing's flag should be the wa the IdP sent (wsignout1.0 " +
        "/ wsignoutcleanup1.0) or the " +
  ...
  var status = await driver.findElement(By.id("wsfed_resp_status"))
      .getAttribute("value");
  log.info("Sign-out landing: " + status);

  // The Logout (Sign Out) button's operation is recorded in the shared history
  // as "Sent" when clicked, then resolved here on the response page. Assert it
  // landed on Success — the user-visible sign that the logout call completed.
  ...
  var leftover = await driver.findElement(By.id("wsfed_response_xml"))
      .getAttribute("value");
  assert(!leftover || leftover.indexOf("RequestSecurityTokenResponse") < 0,
    "a sign-out must not deliver a token, but the response page is " +
        "showing an RSTR.");
  ...
  // sets a status — so make sure both are filled. Without this, a page that
  // came back with empty fields would look like the IdP never answered.
  var again = await driver.executeScript(
    "return { signin: (document.getElementById('wsfed_signin_endpoint') || " +
        "{}).value || ''," +
    "         realm: (document.getElementById('wsfed_realm') || " +
        "{}).value || '' };");
  if (!again.signin) { await setField(driver, "wsfed_signin_endpoint",
      deriveEndpoint(metadataUrl)); }
  ...
  var outcome = await driver.wait(async function () {
    ...
    return null;
  }, loginWait, "after signing out, a second sign-in neither showed a login " +
      "form nor returned a token.");
  ...
  log.info("WS-Federation sign-out succeeded: the IdP reported the sign-out " +
...
async function test() {
  ...
  // the page may be a deployed https site) AND secure context (Validate
  // Signature needs window.crypto.subtle, which does not exist on a plain-http
  // non-localhost origin like the container's http://client:3000). See
  // browser_flags.js.
  ...
  try {
    ...
    // Sign-out needs the session the sign-in just established, so it runs in
    // the same browser, in this order, and only for the dedicated sign-out job.
