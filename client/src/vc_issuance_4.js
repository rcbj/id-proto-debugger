...
var state = {
  ...
  request: { holderKeys: [], nonce: "", proofs: [], body: null,
            encryption: null },
  ...
  refreshed: null,
  // The deferred issuance a refresh can also turn into (section 9). historyId
  // is the log row for the poll that is waiting to be resolved.
...
function describeToken(token) {
  log.debug("Entering describeToken().");
  if (!token) {
    log.debug("Leaving describeToken().");
    return "— none —";
  }
  ...
  if (!claims) {
    log.debug("Leaving describeToken().");
    return head + " (opaque — not a JWT this page can read)";
  }
  log.debug("Leaving describeToken().");
  return head + " — issued " + isoOf(claims.iat) + ", expires " +
      isoOf(claims.exp) +
...
function renderCurrentCredential() {
  ...
  if (!state.current) {
    ...
    status("vc_current_status",
      "There is no credential to refresh. Run steps 1 to 3 first — a refresh " +
          "asks the issuer for a new " +
  ...
  // Type, validity and binding are all read through sd_jwt_vc.js rather than
  // off the payload, because all three are spelled differently by the two
  // families and reading only the SD-JWT spelling reports DIFFERENCE as ABSENCE
  // — the failure that matters, because absence is the reassuring answer. An
  // ldp_vc showed as having no type, never expiring, and bound to no holder
  // key; all three were false. Each label names the member it actually read, so
  // the pane cannot claim to be showing a vct while showing a type array.
  setText("vc_current_vct_label", "Credential type (" +
          sdJwtVc.typeMemberName(parsed) + ")");
  setText("vc_current_vct", sdJwtVc.credentialLabel(parsed) || "—");

  var window_ = sdJwtVc.validityWindowOf(parsed);
  var expired = window_.expires &&
      window_.expires <= Math.floor(Date.now() / 1000);
  ...
  setText("vc_current_validity",
    ...
    (window_.expires ? (expired ? "EXPIRED " + relativeTo(window_.expires)
                                : "valid, expires " +
                                    relativeTo(window_.expires))
                     : "no " + window_.expiresMember +
                         ", so it does not expire on its own"));
  ...
  setText("vc_current_binding", bound
    ? bound.kty + " " + (bound.crv || "") + " key " + String(bound.x ||
        "").slice(0, 16) + "…" +
      (holderKeyIsHeld(bound) ? " — the holder key this browser generated"
                              : " — NOT a holder key this browser still has, " +
                                  "so a proof for it cannot be signed")
  ...
  if (state.current.error) {
    status("vc_current_status", "The stored credential could not be parsed: " +
           state.current.error, "vc-bad");
  } else if (expired) {
    status("vc_current_status",
      "This credential has expired. A refresh is exactly what section 14.5 " +
          "is for — and if the Refresh Token " +
      "has expired too, the issuance has to start again from step 1.",
          "vc-pending");
  } else {
    status("vc_current_status",
      "This credential is still valid. Refreshing it anyway is legitimate: " +
          "section 14.3 says the Credential " +
      "Endpoint may be asked again, and the issuer decides whether the " +
          "answer is the same credential or an " +
      "updated one.", "vc-ok");
  }
...
function holderKeyIsHeld(jwk) {
  ...
  // just pasted here — trusting it would refuse a reuse that is perfectly
  // valid. An EC private JWK carries x/y, so it can answer for itself.
  log.debug("Leaving holderKeyIsHeld().");
...
//
// The rows the wallet HOLDS (outcome kept, with a credential) are the
// generations the navigation moves between; a log row cannot be activated, and
// says so. Being able to go BACK matters: a refresh that turned out worse than
// what it replaced is a real outcome, and a wallet that cannot return to the
...
// generation number, which only the rows the wallet HOLDS have, because only
// those are credentials it can go back to.
function historyTableHead() {
  log.debug("Entering historyTableHead().");
  log.debug("Leaving historyTableHead().");
  return '<table class="vc-table"><thead><tr>' +
         ...
         "<th style='width:11%'>Outcome</th><th style='width:24%'>What " +
             "happened</th>" +
         "<th style='width:18%'>Credential</th><th>Action</th></tr></thead><tbody>";
}

// What the row says about the credential an attempt produced, if it produced
// one.
function credentialCell(entry) {
  ...
  lines.push("expires " + esc(isoOf(s.exp)) + (s.exp ? " (" +
             esc(relativeTo(s.exp)) + ")" : ""));
  lines.push("key " + esc(String(s.boundKey || "—").slice(0, 10)) +
             (s.boundKey ? "…" : ""));
  lines.push("sig " + esc(String(s.signature || "—").slice(0, 10)) +
             (s.signature ? "…" : ""));
...
function historyRow(entry, attemptNumber, generation, isActive) {
  ...
  if (outcome === sdJwtVc.HISTORY_OUTCOME.PENDING) {
    html += '<input class="btn2" type="button" value="Keep" onclick="return ' +
        'vcissuance4.replaceCredential();" /> ' +
...
function renderHistory() {
  ...
  // Which generation is in hand is exactly what the presentation offer names,
  // and every path that changes it comes through here — keeping, discarding,
  ...
  var navButtons = ["vc_history_oldest_button", "vc_history_older_button",
      "vc_history_newer_button",
                    "vc_history_latest_button"];
  if (!history.length) {
    setHtml("vc_history_table", "");
    setText("vc_history_position", "nothing recorded yet");
    navButtons.concat(["vc_history_clear_button"])
                      .forEach(function (id) { disable(id, true); });
    status("vc_history_status",
      "Nothing has been recorded yet. Every attempt on this page — " +
          "refreshing the access token, asking the " +
      "Credential Endpoint, and what you then decided — is added here, and " +
          "this is where you move between the " +
  ...
  setText("vc_history_position",
    (active ? "generation " + active.generation + " of " +
     active.total : "no generation in hand") +
  ...
  // One table, newest attempt first. Deliberately NOT grouped by credential
  // type any more: an access-token refresh has no vct, and a log of attempts
  // only makes sense in the order they were made. The newest HISTORY_LIMIT
  // attempts, newest first. The list is capped rather than allowed to grow
  // without limit, and it says so when it has to cap.
  ...
  shown.slice().reverse().forEach(function (entry, i) {
    ...
    html += historyRow(entry, attemptNumber, generationOf[entry.id],
                       !!(active && active.id === entry.id));
  });
  html += "</tbody></table>";
  if (firstShown > 0) {
    html += '<p class="vc-note">The newest ' + sdJwtVc.HISTORY_LIMIT +
        " attempts are shown; " + firstShown +
            " older one(s) are no longer kept.</p>";
  }
  setHtml("vc_history_table", html);
  log.debug("Leaving renderHistory(). " + history.length + " attempt(s), " +
            shown.length + " shown, " +
            held.length + " generation(s), active " + (active ?
                active.generation : "none") + ".");
  return true;
}

// Everything on the page that depends on WHICH generation is in hand.
// Activating a different one changes the credential the proof of possession
// will be bound to and what a comparison is against, so both are rebuilt rather
// than left showing the previous generation's state.
  ...
  prepareReissue().then(function (ready) {
    ...
    if (ready) {
      ...
      status("vc_reissue_status", reissueMessage ||
        "The request below is built for the generation now in hand, and is " +
            "what will be sent.", "vc-ok");
    }
  });
...
function activateGeneration(id) {
  ...
  if (!entry) {
    status("vc_history_status",
      "That generation is no longer in the history — a log row cannot be " +
          "activated, and a trimmed one is gone.",
      "vc-bad");
  ...
  reloadHeldCredential(
    ...
    (entry.source || entry.kind || "issued") + ", recorded " + (entry.at ||
     "?") +
    ") is now the credential the wallet holds, with the holder key it is " +
        "bound to. " +
    (active.generation < active.total
      ? "It is not the newest one — which is a legitimate place to be, and " +
          "what a wallet does when a refresh " +
...
// Backwards and forwards through the generations the wallet HELD — the log rows
// in between (a refused refresh, a discarded credential) are not places to be.
function historyStep(delta) {
  ...
  if (!held.length || target < 0 || target >= held.length) {
    status("vc_history_status", delta < 0
      ...
      : "This is the newest generation — a refresh above is what adds another.",
          "vc-pending");
...
function historyOlder() {
  log.debug("Entering historyOlder().");
  log.debug("Leaving historyOlder().");
  return historyStep(-1);
}
function historyNewer() {
  log.debug("Entering historyNewer().");
  log.debug("Leaving historyNewer().");
  return historyStep(1);
}
...
function historyOldest() {
  log.debug("Entering historyOldest().");
  log.debug("Leaving historyOldest().");
  return historyJump(false);
}
function historyLatest() {
  log.debug("Entering historyLatest().");
  log.debug("Leaving historyLatest().");
  return historyJump(true);
}

// Forgetting the list is not the same as giving up the credential: whatever is
// in hand stays in hand, and says so.
function clearHistory() {
  log.debug("Entering clearHistory().");
  sdJwtVc.clearCredentialHistory();
  renderHistory();
  status("vc_history_status",
    "The history was cleared. The credential the wallet holds is untouched — " +
        "only the record of the earlier " +
    "generations is gone, so there is nothing left to navigate back to.",
        "vc-pending");
...
// specific is why the wallet wants it — the access token is about to be spent
// on the Credential Endpoint again.
// ---------------------------------------------------------------------------
function refreshParams() {
  ...
  // is whatever the OAuth2 pages were configured with; a public client (which
  // is what the workflow uses by default) has none and sends none.
...
function renderRefreshRequest() {
  ...
  setText("vc_refresh_current_access_token",
          describeToken(sdJwtVc.get("token_access_token") || ""));
  ...
  if (!params) {
    ...
    status("vc_refresh_status",
      "There is no refresh token. The authorization server did not issue one " +
          "— OID4VCI's pre-authorized code " +
      "flow (H.2/H.3) normally does not — so this half cannot run. Section " +
          "14.5's other two routes are still " +
      "open: ask the Credential Endpoint again with the access token below " +
          "(section 14.3), or start the " +
...
function onRefreshScopeChange() {
  log.debug("Entering onRefreshScopeChange().");
  renderRefreshRequest();
  log.debug("Leaving onRefreshScopeChange().");
  return true;
}

// The DPoP context for this page's calls (RFC 9449). Step 4 makes the same two
// calls step 2 does — a Token Request and a Credential Request — so it needs
// the same proofs, and a refresh token bound by RFC 9449 section 5 cannot be
// redeemed without one at all.
//
...
function dpopProblem() {
  log.debug("Entering dpopProblem().");
  if (!sdJwtVc.dpopEnabled()) {
    log.debug("Leaving dpopProblem().");
    return "";
  }
  if (sdJwtVc.dpopContext()) {
    log.debug("Leaving dpopProblem().");
    return "";
  }
  log.debug("Leaving dpopProblem().");
  return "DPoP is on but this browser holds no DPoP key pair, so no proof " +
      "can be made. A " +
         "bound access or refresh token cannot be used without it — " +
             "which is a " +
         "sender-constrained token behaving exactly as intended. Go back to " +
             "step 2 to " +
...
function sendRefreshRequest() {
  ...
  if (!params || !endpoint) {
    status("vc_refresh_status",
           "There is nothing to send: no refresh token, or no token endpoint.",
           "vc-bad");
  ...
  disable("vc_refresh_button", true);
  status("vc_refresh_status",
         "Exchanging the refresh token for a fresh access token …",
         "vc-pending");
  vciWallet.fetchProtected({
    ...
    .then(function (result) {
      if (result.body && result.body.token_type) {
        sdJwtVc.set(sdJwtVc.KEYS.DPOP_TOKEN_TYPE,
                    String(result.body.token_type));
      }
    ...
    .then(function (response) {
      ...
      if (!response.ok || !response.body || !response.body.access_token) {
        ...
        sdJwtVc.recordHistoryEntry({
          ...
          detail: "HTTP " + response.statusCode + " from " + endpoint + " — " +
              err
        });
        renderHistory();
        status("vc_refresh_status",
          "The token endpoint refused the refresh: " + err +
          " A refresh token that has expired or been revoked leaves only one " +
              "route — start the issuance " +
      ...
      sdJwtVc.recordHistoryEntry({
        ...
        detail: "a fresh access token" +
                ...
                (response.body.refresh_token
                  ? (rotated ?
                      "; the refresh token was rotated" : "; the same " +
                      "refresh token came back")
      ...
      renderReissue();
      renderHistory();
      // The proof was signed against the state before the refresh; nothing
      // about it depends on the access token, so it stays valid. Rebuilding the
    ...
    .catch(function (e) {
      ...
      // The call never completed — CORS, DNS, a server that is not there. Still
      // an attempt, and the one hardest to diagnose from memory later.
      sdJwtVc.recordHistoryEntry({
        kind: sdJwtVc.HISTORY_KIND.TOKEN_REFRESH,
        outcome: sdJwtVc.HISTORY_OUTCOME.FAILED,
      ...
      renderHistory();
      status("vc_refresh_status", "The refresh request failed: " + e.message,
             "vc-bad");
      disable("vc_refresh_button", false);
    });
  log.debug("Leaving sendRefreshRequest().");
...
//
// Byte for byte the request step 2 makes, which is why it is built with the
// same module. The one thing a refresh gets to decide differently is the key:
// staying bound to the key the credential in hand already uses is what makes
// the result a REPLACEMENT for it, and generating a new one produces a
// credential bound to something else — useful, but a different thing.
// ---------------------------------------------------------------------------
function keyChoice() {
  log.debug("Entering keyChoice().");
  ...
  if (choice === "reuse" && !holderKeyIsHeld(boundJwk)) {
    log.debug("Leaving keyChoice().");
    return "new";
  }
  log.debug("Leaving keyChoice().");
  return choice;
...
function renderHolderKeyRow() {
  ...
  else if (pasted.jwk) note.textContent =
           "Using the key pasted here. It is not stored.";
  else {
    note.textContent = "Not in this browser's storage — paste the key pair " +
        "you downloaded on step 2 to " +
...
function holderKeyForRequest() {
  ...
  if (keyChoice() === "reuse") {
    ...
    var pub = sdJwtVc.getJson(sdJwtVc.KEYS.HOLDER_JWK) ||
              (priv ? { kty: priv.kty, crv: priv.crv, x: priv.x,
               y: priv.y } : null);
    if (pub && priv) {
      log.debug("Leaving holderKeyForRequest(). Reusing the stored " +
                "holder key.");
      return Promise.resolve({ publicJwk: pub, privateJwk: priv });
    }
  }
  log.debug("Leaving holderKeyForRequest(). Generating a new holder key.");
  // Deliberately NOT written over the wallet's holder key here: the credential
  // in hand is bound to that one, and until the refreshed credential is kept,
  // the private half of it is the only thing that can present the credential
  // the wallet actually has. keepRefreshed() parks the new pair;
  // replaceCredential() promotes it.
...
function prepareReissue() {
  ...
  log.debug("Leaving prepareReissue().");
...
function renderReissueCall() {
  ...
  var text = vciWallet.describeCall({
    ...
    authorization: ((sdJwtVc.dpopEnabled() && dpopContext()) ?
                    "DPoP " : "Bearer ") +
...
function renderReissue() {
  ...
  setText("vc_reissue_access_token",
          describeToken(sdJwtVc.get("token_access_token") || ""));
  var boundJwk = sdJwtVc.boundHolderJwk(state.current && state.current.parsed);
  ...
  if (select) {
    ...
    if (!held) {
      ...
      // can: pasting the downloaded key into the field below makes the bound
      // key usable without it ever being stored. Without this branch the option
  ...
  setText("vc_refresh_key_note", keyChoice() === "reuse"
    ...
    : "A new key pair means the refreshed credential is bound to something " +
        "else: a verifier will demand " +
      "proof of possession of THIS key, and the old credential still needs " +
          "the old one.");
...
function onKeyModeChange() {
  ...
  status("vc_reissue_status", "Rebuilding the Credential Request …",
         "vc-pending");
  prepareReissue().then(function (ready) {
    renderReissue();
    if (ready) {
      status("vc_reissue_status",
        "Ready: the request below is what will be sent to the Credential " +
            "Endpoint.", "vc-ok");
    }
  });
...
function requestRefreshedCredential() {
  ...
  if (!accessToken) {
    status("vc_reissue_status",
      "There is no access token to present. Refresh one above, or run the " +
          "issuance from step 1.", "vc-bad");
  ...
  var send = function () {
    ...
    status("vc_reissue_status",
           "Asking the Credential Endpoint for a refreshed credential …",
           "vc-pending");
    ...
    return vciWallet.fetchProtected({
      ...
      return vciWallet.readCredentialResponse(result.response, result.text,
          null);
    });
  };

  var ready = state.request.proofs.length ?
      Promise.resolve(true) : prepareReissue();
  ready
    .then(send)
    .then(function (response) {
    ...
    .then(function (response) {
      ...
      if (!response.ok && !(response.statusCode === 202 && response.body &&
        ...
        status("vc_reissue_status",
          ...
          (response.statusCode === 401
            ? " A 401 here is section 14.3's other outcome: the issuer has " +
                "decided this access token is no " +
      ...
      if (response.body && response.body.transaction_id &&
          !vciWallet.extractCredential(response.body)) {
      ...
      status("vc_reissue_status",
        "The issuer returned a credential. What changed is below — nothing " +
            "has replaced the credential in " +
        "hand yet, and the Credential History pane lists it as waiting.",
            "vc-ok");
      renderComparison();
    ...
    .catch(function (e) {
      ...
      status("vc_reissue_status", "The Credential Request failed: " + e.message,
             "vc-bad");
      disable("vc_reissue_button", false);
    });
  log.debug("Leaving requestRefreshedCredential().");
...
// Everything worth remembering about a refreshed credential, kept apart from
// the credential in hand until the holder decides (section 14.5).
function keepRefreshed(credential, responseBody, extra) {
  ...
  var meta = {
    ...
    holderJwks: state.request.holderKeys.map(function (k) {
                                             return k.publicJwk; }),
    ...
    refreshed: true,
    refreshGeneration: Number((state.current && state.current.meta &&
                              state.current.meta.refreshGeneration) || 0) + 1,
  ...
  if (historyId) {
    sdJwtVc.updateHistoryEntry(historyId, {
      ...
      detail: "the deferred credential arrived after " + ((extra ||
          {}).deferredAttempts || 1) + " attempt(s)",
    ...
    historyId = sdJwtVc.recordHistoryEntry({
      ...
      detail: (meta.tokenRefreshed
                ...
                : "the issuer returned a credential, on the access token " +
                    "already in hand (section 14.3)") +
              (all.length > 1 ? " — " + all.length + " credentials" : "") +
              (meta.keyMode === "reuse" ?
               "; bound to the same holder key" : "; bound to a NEW " +
               "holder key"),
      credential: credential,
      credentials: all.length ? all : [credential],
      meta: meta,
  ...
  sdJwtVc.setJson(sdJwtVc.KEYS.REFRESHED_CREDENTIALS, all.length ?
                  all : [credential]);
  ...
  if (meta.keyMode !== "reuse" && state.request.holderKeys.length) {
    sdJwtVc.setJson(sdJwtVc.KEYS.REFRESHED_HOLDER_JWK,
                    state.request.holderKeys[0].publicJwk);
    sdJwtVc.setJson(sdJwtVc.KEYS.REFRESHED_HOLDER_PRIVATE_JWK,
                    state.request.holderKeys[0].privateJwk);
  } else {
    sdJwtVc.remove(sdJwtVc.KEYS.REFRESHED_HOLDER_JWK);
    sdJwtVc.remove(sdJwtVc.KEYS.REFRESHED_HOLDER_PRIVATE_JWK);
  }
  state.refreshed = { raw: credential, all: all.length ? all : [credential],
      meta: meta };
...
function beginDeferred(body, statusCode) {
  ...
  sdJwtVc.recordHistoryEntry({
    ...
    detail: "HTTP " + statusCode + " — deferred as transaction " +
  ...
  setText("vc_reissue_transaction_id", state.deferred.transactionId +
    " at " + (deferredEndpoint() ||
        "— no deferred_credential_endpoint is published —"));
  renderDeferredAttempts();
  status("vc_reissue_status",
    "The issuer answered " + statusCode +
        ": the refreshed credential is not ready yet. It asked for " +
    state.deferred.intervalSeconds + "s between attempts; collect it below.",
        "vc-pending");
  disable("vc_reissue_button", false);
...
function pollDeferred() {
  ...
  if (!state.deferred.transactionId) {
    status("vc_reissue_status", "There is no deferred refresh in progress.",
           "vc-bad");
  ...
  status("vc_reissue_status", "Asking the Deferred Credential Endpoint …",
         "vc-pending");
  vciWallet.fetchProtected({
    url: endpoint,
    method: "POST",
    ...
    .then(function (response) {
      ...
      var summary = credential ? "the refreshed credential was issued"
        : (response.body && response.body.transaction_id)
          ? "still pending (interval " + (response.body.interval ||
              state.deferred.intervalSeconds) + "s)"
      ...
      if (!credential) {
        ...
        sdJwtVc.recordHistoryEntry({
          ...
          detail: "attempt " + state.deferred.attempts.length + ": HTTP " +
              response.statusCode + " — " + summary
        });
        renderHistory();
        status("vc_reissue_status", "Attempt " +
               state.deferred.attempts.length + ": " + summary, "vc-pending");
        return;
      }
      // The poll that produced the credential is the row keepRefreshed()
      // resolves.
      state.deferred.historyId = sdJwtVc.recordHistoryEntry({
        kind: sdJwtVc.HISTORY_KIND.DEFERRED_POLL,
        outcome: sdJwtVc.HISTORY_OUTCOME.DEFERRED,
        detail: "attempt " + state.deferred.attempts.length +
            ": the credential was ready"
      });
      keepRefreshed(credential, response.body, {
        deferred: true,
      ...
      // Spent: the issuer invalidates it once the credential has been
      // collected.
      state.deferred.transactionId = "";
      status("vc_reissue_status",
        "The refreshed credential arrived after " +
            state.deferred.attempts.length +
    ...
    .catch(function (e) {
      ...
      renderHistory();
      status("vc_reissue_status", "The deferred request failed: " + e.message,
             "vc-bad");
    });
  log.debug("Leaving pollDeferred().");
  return false;
...
// Pane 4: what changed.
//
// Section 14.5 leaves it to the issuer "whether to update both the signature
// and the claim values, or only the signature", and section 14.3 says the
// answer may even be the same credential over again. None of that can be
...
function renderComparison() {
  ...
  try {
    ...
    status("vc_compare_status",
           "The refreshed credential could not be parsed: " + e.message,
           "vc-bad");
  ...
  if (!before) {
    status("vc_compare_status",
      "The credential in hand could not be parsed, so there is nothing to " +
          "compare against. The refreshed " +
  ...
  setHtml("vc_compare_table",
    "<thead><tr><th style='width:16%'>Member</th><th style='width:38%'>The " +
        "credential in hand</th>" +
    "<th style='width:38%'>What the issuer just returned</th><th " +
        "style='width:8%'>Changed</th></tr></thead>" +
    "<tbody>" + rows.map(function (r) {
      ...
      return "<tr>" +
        ...
        '<td class="' + (changed ? "vc-bad" : "vc-ok") + '">' + (changed ?
            "yes" : "no") + "</td>" +
        "</tr>";
    }).join("") + "</tbody>");
  ...
  setHtml("vc_compare_claims", diffs.length
    ? "<thead><tr><th style='width:16%'>Claim</th><th " +
        "style='width:42%'>Before</th>" +
      "<th style='width:42%'>After</th></tr></thead><tbody>" +
      diffs.map(function (d) {
        return "<tr><td>" + esc(d.name) + '</td><td class="vc-mono">' +
            esc(d.before) +
               '</td><td class="vc-mono">' + esc(d.after) + "</td></tr>";
      }).join("") + "</tbody>"
    : "<tbody><tr><td>Not one disclosed claim VALUE differs — the End-User's " +
        "data is the same. " +
      "The salts and therefore every Disclosure and digest are new " +
          "regardless, because an issuer mints " +
  ...
  var windowChanged = before && ((before.payload ||
  ...
  var keyChanged = before &&
      JSON.stringify(beforeBound) !== JSON.stringify(afterBound);
  var verdict;
  if (sameCredential) {
    verdict = "The issuer returned the very same credential, byte for byte. " +
        "Section 14.3 allows exactly " +
              "that: asking again does not oblige an issuer to change anything.";
  } else if (!diffs.length && signatureChanged) {
    verdict = "Same claim values, new signature" + (windowChanged ?
        " and a new validity window" : "") +
              " — section 14.5's \"only the signature\" case. The credential " +
                  "is a fresh assertion of " +
              "unchanged facts.";
  } else if (diffs.length) {
    verdict = diffs.length + " claim value(s) changed as well as the " +
        "signature — section 14.5's other " +
              "case, where the issuer updates the content too.";
  } else {
    verdict =
        "The credential differs, but not in its signature or its claim values.";
  }
  if (keyChanged) {
    verdict += " It is bound to a DIFFERENT holder key, so it does not " +
        "replace the old credential for a " +
  ...
  setText("vc_compare_refreshed_meta",
    ...
    ", " + (refreshed.meta.keyMode === "reuse" ?
        "bound to the same holder key" : "bound to a new holder key") +
    (refreshed.meta.deferred ?
     ", collected from the Deferred Credential Endpoint" : "") +
    (refreshed.meta.notificationId ? ", notification_id " +
     refreshed.meta.notificationId : ""));
  disable("vc_replace_button", false);
  disable("vc_discard_button", false);
  // A pending refresh is the newest thing on the screen and is not in storage,
  // so the offer has to disown it the moment it appears.
  renderPresentOffer();
...
function replaceCredential() {
  ...
  if (!state.refreshed) {
    status("vc_compare_status", "There is no refreshed credential to promote.",
           "vc-bad");
    log.debug("Leaving replaceCredential().");
    return false;
  }
  // The old one is kept where it can still be looked at — with the key it is
  // bound to, so keeping it means something — but it is no longer the
  // credential the workflow holds.
  if (state.current) {
    ...
    sdJwtVc.setJson(sdJwtVc.KEYS.PREVIOUS_HOLDER_JWK,
                    sdJwtVc.getJson(sdJwtVc.KEYS.HOLDER_JWK));
  ...
  var resolved = pendingId ? sdJwtVc.updateHistoryEntry(pendingId, {
    ...
    source: state.refreshed.meta.tokenRefreshed ?
        "refreshed" : "refreshed (§14.3)",
  ...
  // history, where this generation has just become the one in hand.
  // Deliberately WITHOUT navigating anywhere: the pane the holder just acted in
  // is the pane that has to show what the action did. Step 3 is a button away.
  var total = sdJwtVc.heldGenerations().length;
  reloadHeldCredential(
    "Generation " + total + " is now the credential the wallet holds. The " +
        "one it replaced is still in the " +
    "list, so you can go back to it.",
    "Kept: the wallet now holds the refreshed credential. \u201cVerify in " +
        "step 3\u201d opens it, and the " +
...
// Step 3 verifies whatever the wallet holds. A button rather than a redirect,
// so keeping a credential does not move the browser out from under whoever
// pressed it.
function verifyInStepThree() {
  log.debug("Entering verifyInStepThree().");
  window.location.href = sdJwtVc.STEP3_URL;
  log.debug("Leaving verifyInStepThree().");
  return false;
}

// Hand off to the PRESENTATION workflow. Nothing is copied: that workflow reads
// the same storage keys this page writes, so what it will find is whatever is
// in hand right now — which is the whole reason the offer below spells out
...
function renderPresentOffer() {
  ...
  if (readiness.ready) {
    ...
    if (active && active.total > 1) {
      message += " That is generation " + active.generation + " of " +
          active.total +
        (active.generation < active.total ?
         ", which is not the newest one." : ", the newest one.");
    }
    if (state.refreshed) {
      message += " The refreshed credential below has not been kept, so it " +
          "is NOT what would be presented — " +
...
function forgetRefreshed() {
  log.debug("Entering forgetRefreshed().");
  ...
  sdJwtVc.remove(sdJwtVc.KEYS.REFRESHED_HOLDER_JWK);
  sdJwtVc.remove(sdJwtVc.KEYS.REFRESHED_HOLDER_PRIVATE_JWK);
  log.debug("Leaving forgetRefreshed().");
}

function discardRefreshed() {
  log.debug("Entering discardRefreshed().");
  // The attempt stays in the log — it happened — but the credential itself
  // goes, because "discarded" has to mean discarded. What is left is the record
  // that the issuer returned one and the holder said no.
  var pendingId = ((state.refreshed || {}).meta || {}).historyId || 0;
  if (pendingId) {
    sdJwtVc.updateHistoryEntry(pendingId, {
      outcome: sdJwtVc.HISTORY_OUTCOME.DISCARDED,
      detail: "the issuer returned a credential and it was discarded, so the " +
          "wallet never held it",
  ...
  renderHistory();
  status("vc_reissue_status",
    "The refreshed credential was discarded. The wallet still holds the " +
        "credential from step 3 — which is a " +
    "real choice: an issuer cannot make a wallet keep what it returned.",
        "vc-pending");
...
function onload() {
  ...
  loadCurrentCredential();
  renderCurrentCredential();
  // If the workflow reached here from an older build (or step 2 never ran on
  // this browser) the credential in hand predates the history: record it, so
  // there is a generation 1 to navigate back to rather than a list that starts
  // at the first refresh.
  if (state.current && !sdJwtVc.hasCredentialHistory()) {
    sdJwtVc.recordCredentialGeneration({
      source: (state.current.meta && state.current.meta.refreshGeneration) ?
               "refreshed" : "issued",
      credential: state.current.raw,
      credentials: sdJwtVc.getJson(sdJwtVc.KEYS.CREDENTIALS) ||
                                   [state.current.raw],
  ...
  if (!state.current) {
    ...
    status("vc_reissue_status",
      "There is no credential to refresh yet. The request below is what a " +
          "refresh would send.", "vc-pending");
  }
  prepareReissue().then(function (ready) {
    renderReissue();
